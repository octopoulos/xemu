...
struct xbe* xemu_get_xbe_info(void)
{
	...
	xbe.headers_len = ldl_le_phys(&address_space_memory, hdr_addr_phys + offsetof(struct xbe_header, m_sizeof_headers));
	...
	ssize_t bytes_read = virt_dma_memory_read(hdr_addr_virt, xbe.headers, xbe.headers_len);
