...
#define ISA_LPC47M157_DEVICE(obj) OBJECT_CHECK(ISALPC47M157State, (obj), TYPE_ISA_LPC47M157_DEVICE)
...
static void update_devices(ISALPC47M157State* isa)
{
	...
	for (i = 0; i < ARRAY_SIZE(s->serial); i++)
	{
		...
		iobase = (dev[CONFIG_DEVICE_BASE_ADDRESS_HIGH] << 8) | dev[CONFIG_DEVICE_BASE_ADDRESS_LOW];
		irq = dev[CONFIG_DEVICE_INTERRUPT] & 0x0f;
		if (serial->active && (!dev[CONFIG_DEVICE_ACTIVATE] || serial->iobase != iobase || serial->irq != irq))
		{
		...
		if (!serial->active && dev[CONFIG_DEVICE_ACTIVATE])
		{
			...
			memory_region_init_io(&ss->io, OBJECT(s), &serial_io_ops, ss, "serial", 8);
...
static void lpc47m157_io_write(void* opaque, hwaddr addr, uint64_t val, unsigned int size)
{
	...
	else if (s->configuration_mode)
	{
		...
		else if (addr == DATA_PORT)
		{
			...
			else
			{
				...
				DPRINTF(
					"lpc47m157 dev 0x%02x . 0x%02x = 0x%02" PRIx64 "\n", s->config_regs[CONFIG_DEVICE_NUMBER],
					s->selected_reg, val);
			}
		}
		else
...
static void lpc47m157_realize(DeviceState* dev, Error** errp)
{
	...
	memory_region_init_io(&s->io, OBJECT(s), &lpc47m157_io_ops, isa, TYPE_ISA_LPC47M157_DEVICE, 2);
...
static const VMStateDescription vmstate_lpc47m157 = {
	...
	.fields =
		(VMStateField[]) {
			...
			VMSTATE_STRUCT(state.serial[0].state, ISALPC47M157State, 0, vmstate_serial, SerialState),
			VMSTATE_STRUCT(state.serial[1].state, ISALPC47M157State, 0, vmstate_serial, SerialState),
			VMSTATE_END_OF_LIST(),
		},
};
