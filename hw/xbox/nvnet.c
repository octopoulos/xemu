...
#define NVNET_DEVICE(obj) OBJECT_CHECK(NvNetState, (obj), "nvnet")
...
static void nvnet_mmio_write(void* opaque, hwaddr addr, uint64_t val, unsigned int size);
static uint32_t nvnet_get_reg(NvNetState* s, hwaddr addr, unsigned int size);
static void nvnet_set_reg(NvNetState* s, hwaddr addr, uint32_t val, unsigned int size);
static uint64_t nvnet_io_read(void* opaque, hwaddr addr, unsigned int size);
static void nvnet_io_write(void* opaque, hwaddr addr, uint64_t val, unsigned int size);
...
static ssize_t nvnet_dma_packet_to_guest(NvNetState* s, const uint8_t* buf, size_t size);
static ssize_t nvnet_dma_packet_from_guest(NvNetState* s);
static bool nvnet_can_receive(NetClientState* nc);
static ssize_t nvnet_receive(NetClientState* nc, const uint8_t* buf, size_t size);
static ssize_t nvnet_receive_iov(NetClientState* nc, const struct iovec* iov, int iovcnt);
...
static int nvnet_mii_rw(NvNetState* s, uint64_t val)
{
	...
	int      phy_addr = (mii_ctl >> NVREG_MIICTL_ADDRSHIFT) & 0x1f;
	int      reg = mii_ctl & ((1 << NVREG_MIICTL_ADDRSHIFT) - 1);
	int      write = mii_ctl & NVREG_MIICTL_WRITE;
...
out:
	if (write)
		...
		trace_nvnet_mii_read(phy_addr, reg, nvnet_get_mii_reg_name(reg), retval);
	return retval;
}
	...
	case NvRegTxRxControl:
		...
		else if (val == 0)
		{
			...
			if (temp == NVREG_UNKSETUP3_VAL1)
			{
				...
				nvnet_set_reg(s, NvRegUnknownSetupReg5, NVREG_UNKSETUP5_BIT31, 4);
				break;
			}
		}
...
static ssize_t nvnet_dma_packet_to_guest(NvNetState* s, const uint8_t* buf, size_t size)
{
	...
	nvnet_set_reg(s, NvRegTxRxControl, nvnet_get_reg(s, NvRegTxRxControl, 4) & ~NVREG_TXRXCTL_IDLE, 4);
	...
	for (i = 0; i < s->rx_ring_size; i++)
	{
		...
		NVNET_DPRINTF("RX: Looking at ring descriptor %d (0x%llx): ", s->rx_ring_index, rx_ring_addr);
...
static ssize_t nvnet_dma_packet_from_guest(NvNetState* s)
{
	...
	for (i = 0; i < s->tx_ring_size; i++)
	{
		...
		NVNET_DPRINTF("TX: Looking at ring desc %d (%llx): ", s->tx_ring_index, tx_ring_addr);
		NVNET_DPRINTF("Buffer: 0x%x, ", desc.packet_buffer);
		NVNET_DPRINTF("Length: 0x%x, ", desc.length);
		NVNET_DPRINTF("Flags: 0x%x\n", desc.flags);
		...
		pci_dma_read(d, desc.packet_buffer, &s->tx_dma_buf[s->tx_dma_buf_offset], desc.length + 1);
		...
		desc.flags &=
			~(NV_TX_VALID | NV_TX_RETRYERROR | NV_TX_DEFERRED | NV_TX_CARRIERLOST | NV_TX_LATECOLLISION
...
static void nvnet_realize(PCIDevice* pci_dev, Error** errp)
{
	...
	memory_region_init_io(&s->mmio, OBJECT(dev), &nvnet_mmio_ops, s, "nvnet-mmio", MMIO_SIZE);
	pci_register_bar(d, 0, PCI_BASE_ADDRESS_SPACE_MEMORY, &s->mmio);

	memory_region_init_io(&s->io, OBJECT(dev), &nvnet_io_ops, s, "nvnet-io", IOPORT_SIZE);
	pci_register_bar(d, 1, PCI_BASE_ADDRESS_SPACE_IO, &s->io);

	qemu_macaddr_default_if_unset(&s->conf.macaddr);
	s->nic = qemu_new_nic(&net_nvnet_info, &s->conf, object_get_typename(OBJECT(s)), dev->id, s);
	assert(s->nic);
}
...
static const char* nvnet_get_reg_name(hwaddr addr)
{
	...
	{
	case NvRegIrqStatus:
		return "NvRegIrqStatus";
	case NvRegIrqMask:
		return "NvRegIrqMask";
	case NvRegUnknownSetupReg6:
		return "NvRegUnknownSetupReg6";
	case NvRegPollingInterval:
		return "NvRegPollingInterval";
	case NvRegMisc1:
		return "NvRegMisc1";
	case NvRegTransmitterControl:
		return "NvRegTransmitterControl";
	case NvRegTransmitterStatus:
		return "NvRegTransmitterStatus";
	case NvRegPacketFilterFlags:
		return "NvRegPacketFilterFlags";
	case NvRegOffloadConfig:
		return "NvRegOffloadConfig";
	case NvRegReceiverControl:
		return "NvRegReceiverControl";
	case NvRegReceiverStatus:
		return "NvRegReceiverStatus";
	case NvRegRandomSeed:
		return "NvRegRandomSeed";
	case NvRegUnknownSetupReg1:
		return "NvRegUnknownSetupReg1";
	case NvRegUnknownSetupReg2:
		return "NvRegUnknownSetupReg2";
	case NvRegMacAddrA:
		return "NvRegMacAddrA";
	case NvRegMacAddrB:
		return "NvRegMacAddrB";
	case NvRegMulticastAddrA:
		return "NvRegMulticastAddrA";
	case NvRegMulticastAddrB:
		return "NvRegMulticastAddrB";
	case NvRegMulticastMaskA:
		return "NvRegMulticastMaskA";
	case NvRegMulticastMaskB:
		return "NvRegMulticastMaskB";
	case NvRegTxRingPhysAddr:
		return "NvRegTxRingPhysAddr";
	case NvRegRxRingPhysAddr:
		return "NvRegRxRingPhysAddr";
	case NvRegRingSizes:
		return "NvRegRingSizes";
	case NvRegUnknownTransmitterReg:
		return "NvRegUnknownTransmitterReg";
	case NvRegLinkSpeed:
		return "NvRegLinkSpeed";
	case NvRegUnknownSetupReg5:
		return "NvRegUnknownSetupReg5";
	case NvRegUnknownSetupReg3:
		return "NvRegUnknownSetupReg3";
	case NvRegUnknownSetupReg8:
		return "NvRegUnknownSetupReg8";
	case NvRegUnknownSetupReg7:
		return "NvRegUnknownSetupReg7";
	case NvRegTxRxControl:
		return "NvRegTxRxControl";
	case NvRegMIIStatus:
		return "NvRegMIIStatus";
	case NvRegUnknownSetupReg4:
		return "NvRegUnknownSetupReg4";
	case NvRegAdapterControl:
		return "NvRegAdapterControl";
	case NvRegMIISpeed:
		return "NvRegMIISpeed";
	case NvRegMIIControl:
		return "NvRegMIIControl";
	case NvRegMIIData:
		return "NvRegMIIData";
	case NvRegWakeUpFlags:
		return "NvRegWakeUpFlags";
	case NvRegPatternCRC:
		return "NvRegPatternCRC";
	case NvRegPatternMask:
		return "NvRegPatternMask";
	case NvRegPowerCap:
		return "NvRegPowerCap";
	case NvRegPowerState:
		return "NvRegPowerState";
	default:
		return "Unknown";
	}
}
...
static void nvnet_dump_ring_descriptors(NvNetState* s)
{
	...
	for (int i = 0; i < s->tx_ring_size; i++)
	{
		...
		pci_dma_read(d, tx_ring_addr, &desc, sizeof(desc));
		NVNET_DPRINTF("TX: Dumping ring desc %d (%llx): ", i, tx_ring_addr);
		NVNET_DPRINTF("Buffer: 0x%x, ", desc.packet_buffer);
		NVNET_DPRINTF("Length: 0x%x, ", desc.length);
		NVNET_DPRINTF("Flags: 0x%x\n", desc.flags);
	}
	...
	for (int i = 0; i < s->rx_ring_size; i++)
	{
		...
		pci_dma_read(d, rx_ring_addr, &desc, sizeof(desc));
		NVNET_DPRINTF("RX: Dumping ring desc %d (%llx): ", i, rx_ring_addr);
		NVNET_DPRINTF("Buffer: 0x%x, ", desc.packet_buffer);
		NVNET_DPRINTF("Length: 0x%x, ", desc.length);
		NVNET_DPRINTF("Flags: 0x%x\n", desc.flags);
	}
...
static const VMStateDescription vmstate_nvnet = {
	...
	.fields =
		(VMStateField[]) { VMSTATE_PCI_DEVICE(parent_obj, NvNetState), VMSTATE_UINT8_ARRAY(regs, NvNetState, MMIO_SIZE),
						   VMSTATE_UINT32_ARRAY(phy_regs, NvNetState, 6), VMSTATE_UINT8(tx_ring_index, NvNetState),
						   VMSTATE_UINT8(tx_ring_size, NvNetState), VMSTATE_UINT8(rx_ring_index, NvNetState),
						   VMSTATE_UINT8(rx_ring_size, NvNetState), VMSTATE_END_OF_LIST() },
};
