...
#define MCPX_ACI_DEVICE(obj) OBJECT_CHECK(MCPXACIState, (obj), "mcpx-aci")
...
static void mcpx_aci_realize(PCIDevice* dev, Error** errp)
{
	...
	memory_region_init_io(&d->io_nam, OBJECT(dev), &ac97_io_nam_ops, &d->ac97, "mcpx-aci-nam", 0x100);
	memory_region_init_io(&d->io_nabm, OBJECT(dev), &ac97_io_nabm_ops, &d->ac97, "mcpx-aci-nabm", 0x80);
