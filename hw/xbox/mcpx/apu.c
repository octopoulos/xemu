...
#define MCPX_APU_DEVICE(obj) OBJECT_CHECK(MCPXAPUState, (obj), "mcpx-apu")
...
static uint32_t voice_get_mask(MCPXAPUState* d, uint16_t voice_handle, hwaddr offset, uint32_t mask);
static void voice_set_mask(MCPXAPUState* d, uint16_t voice_handle, hwaddr offset, uint32_t mask, uint32_t val);
static uint64_t mcpx_apu_read(void* opaque, hwaddr addr, unsigned int size);
static void mcpx_apu_write(void* opaque, hwaddr addr, uint64_t val, unsigned int size);
...
static int voice_resample(MCPXAPUState* d, uint16_t v, float samples[][2], int requested_num, float rate);
static void voice_reset_filters(MCPXAPUState* d, uint16_t v);
static void voice_process(MCPXAPUState* d, float mixbins[NUM_MIXBINS][NUM_SAMPLES_PER_FRAME], uint16_t v);
static int voice_get_samples(MCPXAPUState* d, uint32_t v, float samples[][2], int num_samples_requested);
...
static uint32_t voice_get_mask(MCPXAPUState* d, uint16_t voice_handle, hwaddr offset, uint32_t mask)
{
	...
	return (ldl_le_phys(&address_space_memory, voice + offset) & mask) >> ctz32(mask);
}
...
static void voice_set_mask(MCPXAPUState* d, uint16_t voice_handle, hwaddr offset, uint32_t mask, uint32_t val)
{
	...
	stl_le_phys(&address_space_memory, voice + offset, v | ((val << ctz32(mask)) & mask));
}
...
static void fe_method(MCPXAPUState* d, uint32_t method, uint32_t argument)
{
	...
	case NV1BA0_PIO_VOICE_ON:
		...
		{
			unsigned int antecedent_voice = GET_MASK(d->regs[NV_PAPU_FEAV], NV_PAPU_FEAV_VALUE);
		...
		voice_set_mask(d, selected_handle, NV_PAVS_VOICE_PAR_OFFSET, NV_PAVS_VOICE_PAR_OFFSET_CBO, 0);
		...
		voice_set_mask(d, selected_handle, NV_PAVS_VOICE_PAR_STATE, NV_PAVS_VOICE_PAR_STATE_ACTIVE_VOICE, 1);
	...
	case NV1BA0_PIO_SET_VOICE_CFG_VBIN:
		voice_set_mask(d, d->regs[NV_PAPU_FECV], NV_PAVS_VOICE_CFG_VBIN, 0xFFFFFFFF, argument);
		break;
	case NV1BA0_PIO_SET_VOICE_CFG_FMT:
		voice_set_mask(d, d->regs[NV_PAPU_FECV], NV_PAVS_VOICE_CFG_FMT, 0xFFFFFFFF, argument);
		break;
	case NV1BA0_PIO_SET_VOICE_CFG_ENV0:
		voice_set_mask(d, d->regs[NV_PAPU_FECV], NV_PAVS_VOICE_CFG_ENV0, 0xFFFFFFFF, argument);
		break;
	case NV1BA0_PIO_SET_VOICE_CFG_ENVA:
		voice_set_mask(d, d->regs[NV_PAPU_FECV], NV_PAVS_VOICE_CFG_ENVA, 0xFFFFFFFF, argument);
		break;
	case NV1BA0_PIO_SET_VOICE_CFG_ENV1:
		voice_set_mask(d, d->regs[NV_PAPU_FECV], NV_PAVS_VOICE_CFG_ENV1, 0xFFFFFFFF, argument);
		break;
	case NV1BA0_PIO_SET_VOICE_CFG_ENVF:
		voice_set_mask(d, d->regs[NV_PAPU_FECV], NV_PAVS_VOICE_CFG_ENVF, 0xFFFFFFFF, argument);
		break;
	case NV1BA0_PIO_SET_VOICE_CFG_MISC:
		voice_set_mask(d, d->regs[NV_PAPU_FECV], NV_PAVS_VOICE_CFG_MISC, 0xFFFFFFFF, argument);
		break;
	case NV1BA0_PIO_SET_VOICE_TAR_VOLA:
		voice_set_mask(d, d->regs[NV_PAPU_FECV], NV_PAVS_VOICE_TAR_VOLA, 0xFFFFFFFF, argument);
		break;
	case NV1BA0_PIO_SET_VOICE_TAR_VOLB:
		voice_set_mask(d, d->regs[NV_PAPU_FECV], NV_PAVS_VOICE_TAR_VOLB, 0xFFFFFFFF, argument);
		break;
	case NV1BA0_PIO_SET_VOICE_TAR_VOLC:
		voice_set_mask(d, d->regs[NV_PAPU_FECV], NV_PAVS_VOICE_TAR_VOLC, 0xFFFFFFFF, argument);
		break;
	case NV1BA0_PIO_SET_VOICE_LFO_ENV:
		voice_set_mask(d, d->regs[NV_PAPU_FECV], NV_PAVS_VOICE_TAR_LFO_ENV, 0xFFFFFFFF, argument);
		break;
	case NV1BA0_PIO_SET_VOICE_TAR_FCA:
		voice_set_mask(d, d->regs[NV_PAPU_FECV], NV_PAVS_VOICE_TAR_FCA, 0xFFFFFFFF, argument);
		break;
	...
	case NV1BA0_PIO_SET_VOICE_CFG_BUF_LBO:
		voice_set_mask(
			d, d->regs[NV_PAPU_FECV], NV_PAVS_VOICE_CUR_PSH_SAMPLE, NV_PAVS_VOICE_CUR_PSH_SAMPLE_LBO, argument);
		break;
	case NV1BA0_PIO_SET_VOICE_BUF_CBO:
		voice_set_mask(d, d->regs[NV_PAPU_FECV], NV_PAVS_VOICE_PAR_OFFSET, NV_PAVS_VOICE_PAR_OFFSET_CBO, argument);
		break;
	case NV1BA0_PIO_SET_VOICE_CFG_BUF_EBO:
		voice_set_mask(d, d->regs[NV_PAPU_FECV], NV_PAVS_VOICE_PAR_NEXT, NV_PAVS_VOICE_PAR_NEXT_EBO, argument);
		break;
	...
	case NV1BA0_PIO_SET_CURRENT_INBUF_SGE_OFFSET:
	{
		...
		break;
		CASE_4(NV1BA0_PIO_SET_OUTBUF_LEN, 8)
			: // 8 byte pitch, 4 entries
#ifdef DEBUG_MCPX
	...
	case NV1BA0_PIO_SET_SSL_SEGMENT_OFFSET ... NV1BA0_PIO_SET_SSL_SEGMENT_LENGTH + 8 * 64 - 1:
	{
	...
	case NV1BA0_PIO_SET_HRTF_HEADROOM:
		...
		break;
	case NV1BA0_PIO_SET_SUBMIX_HEADROOM ... NV1BA0_PIO_SET_SUBMIX_HEADROOM + 4 * (NUM_MIXBINS - 1):
		assert((method & 3) == 0);
		slot = (method - NV1BA0_PIO_SET_SUBMIX_HEADROOM) / 4;
		d->vp.submix_headroom[slot] = argument & NV1BA0_PIO_SET_SUBMIX_HEADROOM_AMOUNT;
		break;
	...
	MCPXAPUState* d, hwaddr sge_base, unsigned int max_sge, uint8_t* ptr, uint32_t addr, size_t len, bool dir)
{
	...
	while (len > 0)
	{
		...
		uint32_t prd_address = ldl_le_phys(&address_space_memory, sge_base + page_entry * 8 + 0);
...
{
	MCPXAPUState* d = opaque;
	// fprintf(stderr, "GP %s scratch 0x%x bytes (0x%x words) at %x (0x%x words)\n", dir ? "writing to" : "reading
	// from", len, len/4, addr, addr/4);
	scatter_gather_rw(d, d->regs[NV_PAPU_GPSADDR], d->regs[NV_PAPU_GPSMAXSGE], ptr, addr, len, dir);
}
...
static void gp_fifo_rw(void* opaque, uint8_t* ptr, unsigned int index, size_t len, bool dir)
{
	...
	if (dir)
	{
		...
		base = GET_MASK(d->regs[NV_PAPU_GPOFBASE0 + 0x10 * index], NV_PAPU_GPOFBASE0_VALUE);
		end = GET_MASK(d->regs[NV_PAPU_GPOFEND0 + 0x10 * index], NV_PAPU_GPOFEND0_VALUE);
		cur_reg = NV_PAPU_GPOFCUR0 + 0x10 * index;
	}
	else
	{
		assert(index < GP_INPUT_FIFO_COUNT);
		base = GET_MASK(d->regs[NV_PAPU_GPIFBASE0 + 0x10 * index], NV_PAPU_GPOFBASE0_VALUE);
		end = GET_MASK(d->regs[NV_PAPU_GPIFEND0 + 0x10 * index], NV_PAPU_GPOFEND0_VALUE);
...
static bool ep_sink_samples(MCPXAPUState* d, uint8_t* ptr, size_t len)
{
	if (d->mon == MCPX_APU_DEBUG_MON_AC97)
		return false;
	else if ((d->mon == MCPX_APU_DEBUG_MON_EP) || (d->mon == MCPX_APU_DEBUG_MON_GP_OR_EP))
	{
...
static void ep_fifo_rw(void* opaque, uint8_t* ptr, unsigned int index, size_t len, bool dir)
{
	...
	if (dir)
	{
		...
		base = GET_MASK(d->regs[NV_PAPU_EPOFBASE0 + 0x10 * index], NV_PAPU_GPOFBASE0_VALUE);
		end = GET_MASK(d->regs[NV_PAPU_EPOFEND0 + 0x10 * index], NV_PAPU_GPOFEND0_VALUE);
		cur_reg = NV_PAPU_EPOFCUR0 + 0x10 * index;
	}
	else
	{
		assert(index < EP_INPUT_FIFO_COUNT);
		base = GET_MASK(d->regs[NV_PAPU_EPIFBASE0 + 0x10 * index], NV_PAPU_GPOFBASE0_VALUE);
		end = GET_MASK(d->regs[NV_PAPU_EPIFEND0 + 0x10 * index], NV_PAPU_GPOFEND0_VALUE);
...
static void set_notify_status(MCPXAPUState* d, uint32_t v, int notifier, int status)
{
	...
	notify_offset += 16 * (MCPX_HW_NOTIFIER_BASE_OFFSET + v * MCPX_HW_NOTIFIER_COUNT + notifier);
	...
	qatomic_or(&d->regs[NV_PAPU_ISTS], NV_PAPU_ISTS_FEVINTSTS | NV_PAPU_ISTS_FENINTSTS);
	d->set_irq = true;
}
...
static long voice_resample_callback(void* cb_data, float** data)
{
	...
	{
		int active = voice_get_mask(d, v, NV_PAVS_VOICE_PAR_STATE, NV_PAVS_VOICE_PAR_STATE_ACTIVE_VOICE);
		if (!active)
			break;
	...
	if (sample_count < NUM_SAMPLES_PER_FRAME)
	{
		...
		memset(&filter->resample_buf[2 * sample_count], 0, 2 * (NUM_SAMPLES_PER_FRAME - sample_count) * sizeof(float));
...
static int voice_resample(MCPXAPUState* d, uint16_t v, float samples[][2], int requested_num, float rate)
{
	...
	if (filter->resampler == NULL)
	{
		...
		filter->resampler = src_callback_new(&voice_resample_callback, SRC_SINC_FASTEST, 2, &err, filter);
		if (filter->resampler == NULL)
		{
	...
	int count = src_callback_read(filter->resampler, rate, requested_num, (float*)samples);
	if (count == -1)
...
static void voice_process(MCPXAPUState* d, float mixbins[NUM_MIXBINS][NUM_SAMPLES_PER_FRAME], uint16_t v)
{
	...
	float ef_value = voice_step_envelope(
		d, v, NV_PAVS_VOICE_CFG_ENV1, NV_PAVS_VOICE_CFG_ENVF, NV_PAVS_VOICE_CFG_MISC,
		NV_PAVS_VOICE_CFG_MISC_EF_RELEASERATE, NV_PAVS_VOICE_PAR_NEXT, NV_PAVS_VOICE_PAR_NEXT_EFLVL,
	...
	int8_t ps = voice_get_mask(d, v, NV_PAVS_VOICE_CFG_ENV0, NV_PAVS_VOICE_CFG_ENV0_EF_PITCHSCALE);
	float rate = 1.0 / powf(2.0f, (p + ps * 32 * ef_value) / 4096.0f);
	dbg->rate = rate;

	float ea_value = voice_step_envelope(
		d, v, NV_PAVS_VOICE_CFG_ENV0, NV_PAVS_VOICE_CFG_ENVA, NV_PAVS_VOICE_TAR_LFO_ENV,
		NV_PAVS_VOICE_TAR_LFO_ENV_EA_RELEASERATE, NV_PAVS_VOICE_PAR_OFFSET, NV_PAVS_VOICE_PAR_OFFSET_EALVL,
	...
	for (int sample_count = 0; sample_count < NUM_SAMPLES_PER_FRAME;)
	{
		...
		int count = voice_resample(d, v, &samples[sample_count], NUM_SAMPLES_PER_FRAME - sample_count, rate);
		if (count < 0)
			break;
	...
	uint16_t vol[8];
	vol[0] = voice_get_mask(d, v, NV_PAVS_VOICE_TAR_VOLA, NV_PAVS_VOICE_TAR_VOLA_VOLUME0);
	vol[1] = voice_get_mask(d, v, NV_PAVS_VOICE_TAR_VOLA, NV_PAVS_VOICE_TAR_VOLA_VOLUME1);
	vol[2] = voice_get_mask(d, v, NV_PAVS_VOICE_TAR_VOLB, NV_PAVS_VOICE_TAR_VOLB_VOLUME2);
	vol[3] = voice_get_mask(d, v, NV_PAVS_VOICE_TAR_VOLB, NV_PAVS_VOICE_TAR_VOLB_VOLUME3);
	vol[4] = voice_get_mask(d, v, NV_PAVS_VOICE_TAR_VOLC, NV_PAVS_VOICE_TAR_VOLC_VOLUME4);
	vol[5] = voice_get_mask(d, v, NV_PAVS_VOICE_TAR_VOLC, NV_PAVS_VOICE_TAR_VOLC_VOLUME5);

	vol[6] = voice_get_mask(d, v, NV_PAVS_VOICE_TAR_VOLC, NV_PAVS_VOICE_TAR_VOLC_VOLUME6_B11_8) << 8;
	vol[6] |= voice_get_mask(d, v, NV_PAVS_VOICE_TAR_VOLB, NV_PAVS_VOICE_TAR_VOLB_VOLUME6_B7_4) << 4;
	vol[6] |= voice_get_mask(d, v, NV_PAVS_VOICE_TAR_VOLA, NV_PAVS_VOICE_TAR_VOLA_VOLUME6_B3_0);
	vol[7] = voice_get_mask(d, v, NV_PAVS_VOICE_TAR_VOLC, NV_PAVS_VOICE_TAR_VOLC_VOLUME7_B11_8) << 8;
	vol[7] |= voice_get_mask(d, v, NV_PAVS_VOICE_TAR_VOLB, NV_PAVS_VOICE_TAR_VOLB_VOLUME7_B7_4) << 4;
	vol[7] |= voice_get_mask(d, v, NV_PAVS_VOICE_TAR_VOLA, NV_PAVS_VOICE_TAR_VOLA_VOLUME7_B3_0);
	...
	int fmode = voice_get_mask(d, v, NV_PAVS_VOICE_CFG_MISC, NV_PAVS_VOICE_CFG_MISC_FMODE);
	...
	if (lpf)
	{
		for (int ch = 0; ch < 2; ch++)
		{
			...
			int16_t fc = voice_get_mask(d, v, NV_PAVS_VOICE_TAR_FCA + (ch % channels) * 4, NV_PAVS_VOICE_TAR_FCA_FC0);
			float fc_f = clampf(pow(2, fc / 4096.0), 0.003906f, 1.0f);
			uint16_t q = voice_get_mask(d, v, NV_PAVS_VOICE_TAR_FCA + (ch % channels) * 4, NV_PAVS_VOICE_TAR_FCA_FC1);
...
static int voice_get_samples(MCPXAPUState* d, uint32_t v, float samples[][2], int num_samples_requested)
{
	...
	if (stream)
	{
		if (!persist)
		{
			...
			int eacur = voice_get_mask(d, v, NV_PAVS_VOICE_PAR_STATE, NV_PAVS_VOICE_PAR_STATE_EACUR);
			if (eacur < NV_PAVS_VOICE_PAR_STATE_EFCUR_RELEASE)
			{
				DPRINTF(
					"Voice %d envelope not in release state (%d) and "
		...
		if (count == 0)
		{
			...
			voice_set_mask(d, v, NV_PAVS_VOICE_PAR_OFFSET, NV_PAVS_VOICE_PAR_OFFSET_CBO, 0);
	...
	bool adpcm = (container_size_index == NV_PAVS_VOICE_CFG_FMT_CONTAINER_SIZE_ADPCM);
	...
	int sample_count = 0;
	for (; (sample_count < num_samples_requested) && (cbo <= ebo); sample_count++, cbo++)
	{
		if (adpcm)
		{
			...
			if (adpcm_block_index != block_index)
			{
				...
				{
					linear_addr += ba;
					for (unsigned int word_index = 0; word_index < (9 * samples_per_block); word_index++)
					{
						hwaddr addr = get_data_ptr(d->regs[NV_PAPU_VPSGEADDR], 0xFFFFFFFF, linear_addr);
						adpcm_block[word_index] = ldl_le_phys(&address_space_memory, addr);
						linear_addr += 4;
					}
				}
				adpcm_decode_block(adpcm_decoded, (uint8_t*)adpcm_block, block_size, channels);
				adpcm_block_index = block_index;
			}

			samples[sample_count][0] = int16_to_float(adpcm_decoded[block_position * channels]);
			if (stereo)
				samples[sample_count][1] = int16_to_float(adpcm_decoded[block_position * channels + 1]);
		}
		else
		{
			...
			else
			{
				...
				addr = get_data_ptr(d->regs[NV_PAPU_VPSGEADDR], 0xFFFFFFFF, linear_addr);
			}
...
static void se_frame(MCPXAPUState* d)
{
	...
	if (now - d->frame_count_time >= 1000)
	{
		...
		float t = 1.0f - ((double)d->sleep_acc / (double)((now - d->frame_count_time) * 1000));
		g_dbg.utilization = t;
	...
	if (d->mon == MCPX_APU_DEBUG_MON_VP)
	{
		...
		src_float_to_short_array((float*)d->vp.sample_buf, isamp, NUM_SAMPLES_PER_FRAME * 2);
	...
	for (int mixbin = 0; mixbin < NUM_MIXBINS; mixbin++)
	{
		...
		for (int sample = 0; sample < NUM_SAMPLES_PER_FRAME; sample++)
			dsp_write_memory(d->gp.dsp, 'X', base + sample, float_to_24b(mixbins[mixbin][sample]));
	}

	bool ep_enabled =
		(d->ep.regs[NV_PAPU_EPRST] & NV_PAPU_GPRST_GPRST) && (d->ep.regs[NV_PAPU_EPRST] & NV_PAPU_GPRST_GPDSPRST);

	/* Run GP */
	if ((d->gp.regs[NV_PAPU_GPRST] & NV_PAPU_GPRST_GPRST) && (d->gp.regs[NV_PAPU_GPRST] & NV_PAPU_GPRST_GPDSPRST))
	{
		...
		if ((d->mon == MCPX_APU_DEBUG_MON_GP) || (d->mon == MCPX_APU_DEBUG_MON_GP_OR_EP && !ep_enabled))
		{
			int off = (d->ep_frame_div % 8) * NUM_SAMPLES_PER_FRAME;
			for (int i = 0; i < NUM_SAMPLES_PER_FRAME; i++)
			{
				...
				uint32_t r = dsp_read_memory(d->gp.dsp, 'X', 0x1400 + 1 * 0x20 + i);
	...
	/* Run EP */
	if ((d->ep.regs[NV_PAPU_EPRST] & NV_PAPU_GPRST_GPRST) && (d->ep.regs[NV_PAPU_EPRST] & NV_PAPU_GPRST_GPDSPRST))
	{
	...
	if ((d->ep_frame_div + 1) % 8 == 0)
	{
		...
		fifo8_push_all(&d->vp.out_buf, (uint8_t*)d->apu_fifo_output, sizeof(d->apu_fifo_output));
		qemu_spin_unlock(&d->vp.out_buf_lock);
...
static void mcpx_vp_out_cb(void* opaque, uint8_t* stream, int free_b)
{
	...
	while (to_copy > 0)
	{
		...
		qemu_spin_lock(&s->vp.out_buf_lock);
		const uint8_t* samples = fifo8_pop_buf(&s->vp.out_buf, to_copy, &chunk_len);
...
static void mcpx_apu_realize(PCIDevice* dev, Error** errp)
{
	...
	memory_region_init_io(&d->mmio, OBJECT(dev), &mcpx_apu_mmio_ops, d, "mcpx-apu-mmio", 0x80000);

	memory_region_init_io(&d->vp.mmio, OBJECT(dev), &vp_ops, d, "mcpx-apu-vp", 0x10000);
	memory_region_add_subregion(&d->mmio, 0x20000, &d->vp.mmio);

	memory_region_init_io(&d->gp.mmio, OBJECT(dev), &gp_ops, d, "mcpx-apu-gp", 0x10000);
	memory_region_add_subregion(&d->mmio, 0x30000, &d->gp.mmio);

	memory_region_init_io(&d->ep.mmio, OBJECT(dev), &ep_ops, d, "mcpx-apu-ep", 0x10000);
...
static void mcpx_apu_reset(MCPXAPUState* d)
{
	...
	memset(d->gp.dsp->core.pram_opcache, 0, sizeof(d->gp.dsp->core.pram_opcache));
	memset(d->ep.dsp->core.pram_opcache, 0, sizeof(d->ep.dsp->core.pram_opcache));
	d->set_irq = false;
...
const VMStateDescription vmstate_vp_dsp_dma_state = {
	...
	.fields =
		(VMStateField[]) { VMSTATE_UINT32(configuration, DSPDMAState), VMSTATE_UINT32(control, DSPDMAState),
						   VMSTATE_UINT32(start_block, DSPDMAState), VMSTATE_UINT32(next_block, DSPDMAState),
						   VMSTATE_BOOL(error, DSPDMAState), VMSTATE_BOOL(eol, DSPDMAState), VMSTATE_END_OF_LIST() }
};
...
const VMStateDescription vmstate_vp_dsp_core_state = {
	...
	.fields =
		(VMStateField[]) {
			...
			VMSTATE_UINT16(instr_cycle, dsp_core_t), VMSTATE_UINT32(pc, dsp_core_t),
			VMSTATE_UINT32_ARRAY(registers, dsp_core_t, DSP_REG_MAX), VMSTATE_UINT32_2DARRAY(stack, dsp_core_t, 2, 16),
			...
			VMSTATE_UINT32_ARRAY(periph, dsp_core_t, DSP_PERIPH_SIZE), VMSTATE_UINT32(loop_rep, dsp_core_t),
			VMSTATE_UINT32(pc_on_rep, dsp_core_t), VMSTATE_UINT16(interrupt_state, dsp_core_t),
			VMSTATE_UINT16(interrupt_instr_fetch, dsp_core_t), VMSTATE_UINT16(interrupt_save_pc, dsp_core_t),
			VMSTATE_UINT16(interrupt_counter, dsp_core_t), VMSTATE_UINT16(interrupt_ipl_to_raise, dsp_core_t),
			VMSTATE_UINT16(interrupt_pipeline_count, dsp_core_t), VMSTATE_INT16_ARRAY(interrupt_ipl, dsp_core_t, 12),
			VMSTATE_UINT16_ARRAY(interrupt_is_pending, dsp_core_t, 12), VMSTATE_UINT32(num_inst, dsp_core_t),
			VMSTATE_UINT32(cur_inst_len, dsp_core_t), VMSTATE_UINT32(cur_inst, dsp_core_t),
			VMSTATE_BOOL(executing_for_disasm, dsp_core_t), VMSTATE_UINT32(disasm_memory_ptr, dsp_core_t),
			VMSTATE_BOOL(exception_debugging, dsp_core_t), VMSTATE_UINT32(disasm_prev_inst_pc, dsp_core_t),
			VMSTATE_BOOL(disasm_is_looping, dsp_core_t), VMSTATE_UINT32(disasm_cur_inst, dsp_core_t),
...
static const VMStateDescription vmstate_mcpx_apu = {
	...
	.fields =
		(VMStateField[]) {
			...
			VMSTATE_UINT32_ARRAY(ep.regs, MCPXAPUState, 0x10000), VMSTATE_UINT32_ARRAY(regs, MCPXAPUState, 0x20000),
			VMSTATE_UINT32(inbuf_sge_handle, MCPXAPUState), VMSTATE_UINT32(outbuf_sge_handle, MCPXAPUState),
			VMSTATE_STRUCT_ARRAY(vp.ssl, MCPXAPUState, MCPX_HW_MAX_VOICES, 1, vmstate_vp_ssl_data, MCPXAPUVPSSLData),
			VMSTATE_INT32(vp.ssl_base_page, MCPXAPUState), VMSTATE_UINT8_ARRAY(vp.hrtf_submix, MCPXAPUState, 4),
			VMSTATE_UINT8(vp.hrtf_headroom, MCPXAPUState),
			VMSTATE_UINT8_ARRAY(vp.submix_headroom, MCPXAPUState, NUM_MIXBINS),
			VMSTATE_UINT64_ARRAY(vp.voice_locked, MCPXAPUState, 4), VMSTATE_END_OF_LIST() },
};
...
void mcpx_apu_init(PCIBus* bus, int devfn, MemoryRegion* ram)
{
	...
	qemu_thread_create(&d->apu_thread, "mcpx.apu_thread", mcpx_apu_frame_thread, d, QEMU_THREAD_JOINABLE);
}
