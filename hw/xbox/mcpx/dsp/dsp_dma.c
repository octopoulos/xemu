...
static void scratch_circular_copy(
	DSPDMAState* s, uint32_t scratch_base, uint32_t* scratch_offset, uint32_t scratch_size, uint32_t transfer_size,
	uint8_t* scratch_buf, int direction)
{
...
{
	if (!(s->control & DMA_CONTROL_RUNNING) || (s->control & DMA_CONTROL_FROZEN))
		return;
		...
		if (direction)
		{
			if (dsp_interleave)
			{
				...
				for (int i = 0; i < block_count; i++)
				{
					...
					{
						uint32_t v = dsp56k_read_memory(s->core, mem_space, mem_address + ch * block_count + i);
						switch (item_size)
						{
						case 2:
