...
#define CHIHIRO_MACHINE(obj) OBJECT_CHECK(ChihiroMachineState, (obj), TYPE_CHIHIRO_MACHINE)

#define CHIHIRO_MACHINE_CLASS(klass) OBJECT_CLASS_CHECK(ChihiroMachineClass, (klass), TYPE_CHIHIRO_MACHINE)
...
#define CHIHIRO_LPC_DEVICE(obj) OBJECT_CHECK(ChihiroLPCState, (obj), "chihiro-lpc")

static uint64_t chhiro_lpc_io_read(void* opaque, hwaddr addr, unsigned size)
{
	uint64_t r = 0;
	switch (addr)
	{
...
static void chhiro_lpc_io_write(void* opaque, hwaddr addr, uint64_t val, unsigned size)
{
#ifdef DEBUG_CHIHIRO
...
static void chihiro_lpc_realize(DeviceState* dev, Error** errp)
{
	...
	memory_region_init_io(&s->ioport, OBJECT(dev), &chihiro_lpc_io_ops, s, "chihiro-lpc-io", 0x100);
	...
	static void chihiro_ide_interface_init(const char* rom_file, const char* filesystem_file)
{
	...
	memory_region_init(interface, NULL, "chihiro.interface", (uint64_t)0x10000000 * SECTOR_SIZE);

	rom = g_malloc(sizeof(*rom));
	memory_region_init_ram(rom, NULL, "chihiro.interface.rom", ROM_SECTORS * SECTOR_SIZE, &error_fatal);
	memory_region_add_subregion(interface, (uint64_t)ROM_START * SECTOR_SIZE, rom);

	/* limited by the size of the board ram, which we emulate as 128M for now */
	filesystem = g_malloc(sizeof(*filesystem));
	memory_region_init_ram(filesystem, NULL, "chihiro.interface.filesystem", 128 * 1024 * 1024, &error_fatal);
	memory_region_add_subregion(interface, (uint64_t)FILESYSTEM_START * SECTOR_SIZE, filesystem);
...
{
	const char* mediaboard_rom_file = object_property_get_str(qdev_get_machine(), "mediaboard-rom", NULL);
	const char* mediaboard_filesystem_file = object_property_get_str(qdev_get_machine(), "mediaboard-filesystem", NULL);
	chihiro_ide_interface_init(mediaboard_rom_file, mediaboard_filesystem_file);
...
static void chihiro_machine_options(MachineClass* m)
{
	...
	m->no_floppy = 1, m->no_cdrom = 1, m->no_sdcard = 1, m->default_cpu_type = X86_CPU_TYPE_NAME("486");
