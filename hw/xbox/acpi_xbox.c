...
static void xbox_pm_gpio_write(void* opaque, hwaddr addr, uint64_t val, unsigned width)
{
...
static void pm_update_sci(XBOX_PMRegs* pm)
{
	...
	sci_level =
		(((pm1a_sts & pm->acpi_regs.pm1.evt.en)
		  & (ACPI_BITMASK_RT_CLOCK_ENABLE | ACPI_BITMASK_POWER_BUTTON_ENABLE | ACPI_BITMASK_GLOBAL_LOCK_ENABLE
	...
	acpi_pm_tmr_update(
		&pm->acpi_regs,
		(pm->acpi_regs.pm1.evt.en & ACPI_BITMASK_TIMER_ENABLE) && !(pm1a_sts & ACPI_BITMASK_TIMER_STATUS));
}
...
static void xbox_pm_gpe_writeb(void* opaque, hwaddr addr, uint64_t val, unsigned width)
{
	XBOX_PMRegs* pm = opaque;
...
#define VMSTATE_GPE_ARRAY(_field, _state) \
	{ \
		.name = (stringify(_field)), .version_id = 0, .num = XBOX_PM_GPE_LEN, .info = &vmstate_info_uint8, \
		.size = sizeof(uint8_t), .flags = VMS_ARRAY | VMS_POINTER, \
...
const VMStateDescription vmstate_xbox_pm = {
	...
	.fields =
		(VMStateField[]) {
			VMSTATE_UINT16(acpi_regs.pm1.evt.sts, XBOX_PMRegs), VMSTATE_UINT16(acpi_regs.pm1.evt.en, XBOX_PMRegs),
			VMSTATE_UINT16(acpi_regs.pm1.cnt.cnt, XBOX_PMRegs), VMSTATE_TIMER_PTR(acpi_regs.tmr.timer, XBOX_PMRegs),
			VMSTATE_INT64(acpi_regs.tmr.overflow_time, XBOX_PMRegs), VMSTATE_GPE_ARRAY(acpi_regs.gpe.sts, XBOX_PMRegs),
			VMSTATE_GPE_ARRAY(acpi_regs.gpe.en, XBOX_PMRegs), VMSTATE_END_OF_LIST() },
};
...
void xbox_pm_init(PCIDevice* dev, XBOX_PMRegs* pm, qemu_irq sci_irq)
{
	...
	memory_region_init_io(&pm->io_gpe, OBJECT(dev), &xbox_pm_gpe_ops, pm, "xbox-pm-gpe0", XBOX_PM_GPE_LEN);
	memory_region_add_subregion(&pm->io, XBOX_PM_GPE_BASE, &pm->io_gpe);

	memory_region_init_io(&pm->io_gpio, OBJECT(dev), &xbox_pm_gpio_ops, pm, "xbox-pm-gpio", XBOX_PM_GPIO_LEN);
