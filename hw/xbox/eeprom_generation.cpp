...
static void xbox_rc4_crypt(RC4Context* ctx, uint8_t* data, size_t len)
{
	for (size_t i = 0, j = 0, k = 0; k < len; k++)
	{
		...
		j = (j + ctx->s[i]) & 255;
		xbox_rc4_swap(ctx, i, j);
		data[k] ^= ctx->s[(ctx->s[i] + ctx->s[j]) & 255];
	}
}
...
static void xbox_sha1_fill(SHA1Context* ctx, uint32_t a, uint32_t b, uint32_t c, uint32_t d, uint32_t e)
{
...
static void xbox_sha1_reset(SHA1Context* ctx, XboxEEPROMVersion ver, bool first)
{
	...
	case XBOX_EEPROM_VERSION_D:
		if (first)
			xbox_sha1_fill(ctx, 0x85F9E51A, 0xE04613D2, 0x6D86A50C, 0x77C32E3C, 0x4BD717A4);
		else
			xbox_sha1_fill(ctx, 0x5D7A9C6B, 0xE1922BEB, 0xB82CCDBC, 0x3137AB34, 0x486B52B3);
		break;
	case XBOX_EEPROM_VERSION_R2:
		if (first)
			xbox_sha1_fill(ctx, 0x39B06E79, 0xC9BD25E8, 0xDBC6B498, 0x40B4389D, 0x86BBD7ED);
		else
			xbox_sha1_fill(ctx, 0x9B49BED3, 0x84B430FC, 0x6B8749CD, 0xEBFE5FE5, 0xD96E7393);
		break;
	case XBOX_EEPROM_VERSION_R3:
		if (first)
			xbox_sha1_fill(ctx, 0x8058763A, 0xF97D4E0E, 0x865A9762, 0x8A3D920D, 0x08995B2C);
		else
			xbox_sha1_fill(ctx, 0x01075307, 0xA2f1E037, 0x1186EEEA, 0x88DA9992, 0x168A5609);
		break;
	default: // default to 1.0 version
		if (first)
			xbox_sha1_fill(ctx, 0x72127625, 0x336472B9, 0xBE609BEA, 0xF55E226B, 0x99958DAC);
		else
			xbox_sha1_fill(ctx, 0x76441D41, 0x4DE82659, 0x2E8EF85E, 0xB256FACA, 0xC4FE2DE8);
	}
}
...
static void xbox_sha1_compute(SHA1Context* ctx, XboxEEPROMVersion ver, uint8_t* data, size_t len, uint8_t* hash)
{
...
bool xbox_eeprom_generate(const char* file, XboxEEPROMVersion ver)
{
	...
	for (size_t i = 0; i < sizeof(e.serial); i++)
