...
// {
	...
	*/
	...
	template <typename T>
	void GetIndices(
		std::vector<T>& indices, const u32 idxAddr, const u32 idxCount, bool is_primitive_restart_enabled,
	 ...
	 */
	std::string SavePNG(
		std::string string, const int width, const int height, const int channel, const u8* data, const int pitch)
	{
	 ...
	 */
	void GLTF_Accessor(
		int bufferView, int byteOffset, int byteStride, int componentType, int count, rapidjson::Value& accessorMin,
		rapidjson::Value& accessorMax, int size)
	{
	...
	void GLTF_SaveVertexAttributes()
	{
		...
		{
			// save buffer
			auto baseName = fmt::format(
				"-{}{}-{}-{}.bin", drawnCommands[drawnCommand], va.id, va.inSize, int(va.type));
	...
	void NewDrawEnd()
	{
		...
		// vertex
		{
			...
			rapidjson::Value node(rapidjson::kObjectType);
			node.AddMember(
				"name", CreateString(fmt::format("node_{}_{}_{}", nodeId, vertexCount, indexCount / 3)), allocator);
			if (nodeId > 0)
