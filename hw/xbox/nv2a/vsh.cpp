...
static int vsh_get_from_token(const uint32_t* shader_token, uint8_t subtoken, uint8_t start_bit, uint8_t bit_length)
{
...
static std::string decode_swizzle(const uint32_t* shader_token, uint8_t swizzle_field)
{
	...
	if (x == SWIZZLE_X && y == SWIZZLE_Y && z == SWIZZLE_Z && w == SWIZZLE_W)
	{
		...
		return fmt::format(".{}{}", swizzle_str[x], swizzle_str[y]);
	else if (z == w)
		return fmt::format(".{}{}{}", swizzle_str[x], swizzle_str[y], swizzle_str[z]);
	else
	{
		// Normal swizzle mask
		return fmt::format(".{}{}{}{}", swizzle_str[x], swizzle_str[y], swizzle_str[z], swizzle_str[w]);
	}
}

static std::string decode_opcode_input(const uint32_t* shader_token, uint8_t param, uint8_t neg_field, int reg_num)
{
	...
	// PARAM_R uses the supplied reg_num, but the other two need to be determined
	switch (param)
	{
	case PARAM_R:
		ret_str << fmt::format("R{}", reg_num);
		break;
	case PARAM_V:
		reg_num = vsh_get_field(shader_token, FLD_V);
		ret_str << fmt::format("v{}", reg_num);
		break;
	case PARAM_C:
		...
		if (vsh_get_field(shader_token, FLD_A0X) > 0)
		{
			...
			ret_str << fmt::format("c[A0+{}]", reg_num);
		}
		else
			ret_str << fmt::format("c[{}]", reg_num);
		break;
	default:
...

	{
		...
		ret_str << decode_swizzle(shader_token, neg_field + 1);
	}
	...
	const uint32_t* shader_token, VshOutputMux out_mux, uint32_t mask, const char* opcode, std::string inputs)
{
	...
	else if (out_mux == OMUX_ILU && vsh_get_field(shader_token, FLD_MAC) != MAC_NOP)
	{
	...
	if (vsh_get_field(shader_token, FLD_OUT_MUX) == out_mux && vsh_get_field(shader_token, FLD_OUT_O_MASK) != 0)
	{
		ret << "  " << opcode << "(";
		...
		if (vsh_get_field(shader_token, FLD_OUT_ORB) == OUTPUT_C)
		{
			...
			ret << "c" << convert_c_register(vsh_get_field(shader_token, FLD_OUT_ADDRESS));
		}
		else
			ret << out_reg_name[vsh_get_field(shader_token, FLD_OUT_ADDRESS) & 0xF];

		ret << mask_str[vsh_get_field(shader_token, FLD_OUT_O_MASK)] << inputs << ");\n";
	}

	if (strcmp(opcode, mac_opcode[MAC_ARL]) == 0)
		ret << fmt::format("  ARL(A0{});\n", inputs);
	else if (mask > 0)
...
static std::string decode_token(const uint32_t* shader_token)
{
	...
	uint8_t mac = vsh_get_field(shader_token, FLD_MAC);
	// See if a ILU opcode is present too:
	uint8_t ilu = vsh_get_field(shader_token, FLD_ILU);
