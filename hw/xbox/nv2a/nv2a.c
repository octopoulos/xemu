...
void* nv_dma_map(NV2AState* d, hwaddr dma_obj_address, hwaddr* len)
{
	...
	trace_nv2a_dma_map(dma_obj_address, dma.dma_class, dma.dma_target, dma.address, dma.limit);
...
static int nv2a_get_bpp(VGACommonState* s)
{
	...
	case 0:
		...
		break;
	case 2:
		bpp = d->pramdac.general_control & NV_PRAMDAC_GENERAL_CONTROL_ALT_MODE_SEL ? 16 : 15;
		break;
	case 3:
		bpp = 32;
		break;
...
static void nv2a_get_offsets(VGACommonState* s, uint32_t* pline_offset, uint32_t* pstart_addr, uint32_t* pline_compare)
{
	NV2AState* d = container_of(s, NV2AState, vga);
	uint32_t start_addr, line_offset, line_compare;

	line_offset = s->cr[0x13] | ((s->cr[0x19] & 0xe0) << 3) | ((s->cr[0x25] & 0x20) << 6);
	line_offset <<= 3;
