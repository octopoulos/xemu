...
static void nv2a_profile_increment(void)
{
	...
	// intercept frame
	{
		...
		else if (asked && !interceptFrame)
		{
			interceptFrame = asked;
			SetIntercept(1 + interceptFrame * 2);
			NewFrame(true);
		}
		else if (interceptFrame)
		{
			interceptFrame &= ~3;
			NewFrame(false);
		}
...
static void nv2a_profile_flip_stall(void)
{
	...
	g_nv2a_stats.frame_history[g_nv2a_stats.frame_ptr] = g_nv2a_stats.frame_working;
	g_nv2a_stats.frame_ptr                             = (g_nv2a_stats.frame_ptr + 1) % NV2A_PROF_NUM_FRAMES;
...
const char* nv2a_profile_get_counter_name(uint32_t cnt)
{
...
int nv2a_profile_get_counter_value(uint32_t cnt)
{
	assert(cnt < NV2A_PROF__COUNT);
	uint32_t idx = (g_nv2a_stats.frame_ptr + NV2A_PROF_NUM_FRAMES - 1) % NV2A_PROF_NUM_FRAMES;
...
static const GLenum pgraph_texture_mag_filter_map[] = {
	0, GL_NEAREST, GL_LINEAR, 0, GL_LINEAR // TODO: Convolution filter...
};

static const GLenum pgraph_texture_addr_map[] = {
	0, GL_REPEAT, GL_MIRRORED_REPEAT, GL_CLAMP_TO_EDGE, GL_CLAMP_TO_BORDER, GL_CLAMP_TO_EDGE, // Approximate GL_CLAMP
};
...
static const GLenum pgraph_cull_face_map[] = { 0, GL_FRONT, GL_BACK, GL_FRONT_AND_BACK };
...
static void            pgraph_apply_anti_aliasing_factor(PGRAPHState* pg, uint32_t* width, uint32_t* height);
static void            pgraph_apply_scaling_factor(PGRAPHState* pg, uint32_t* width, uint32_t* height);
static void            pgraph_get_surface_dimensions(PGRAPHState* pg, uint32_t* width, uint32_t* height);
static void            pgraph_update_memory_buffer(NV2AState* d, hwaddr addr, hwaddr size, bool quick);
static void            pgraph_bind_vertex_attributes(NV2AState* d, uint32_t min_element, uint32_t max_element, bool inline_data, uint32_t inline_stride, uint32_t provoking_element);
static uint32_t        pgraph_bind_inline_array(NV2AState* d);
...
static void            convert_yuy2_to_rgb(const uint8_t* line, uint32_t ix, uint8_t* r, uint8_t* g, uint8_t* b);
static void            convert_uyvy_to_rgb(const uint8_t* line, uint32_t ix, uint8_t* r, uint8_t* g, uint8_t* b);
static uint8_t*        convert_texture_data(const TextureShape s, const uint8_t* data, const uint8_t* palette_data, uint32_t width, uint32_t height, uint32_t depth, uint32_t row_pitch, uint32_t slice_pitch);
...
static uint32_t kelvin_map_stencil_op(uint32_t parameter);
static uint32_t kelvin_map_polygon_mode(uint32_t parameter);
static uint32_t kelvin_map_texgen(uint32_t parameter, uint32_t channel);
static void     pgraph_reload_surface_scale_factor(NV2AState* d);

static uint32_t pgraph_rdi_read(PGRAPHState* pg, uint32_t select, uint32_t address)
{
	uint32_t r = 0;
	switch (select)
	{
...
static void pgraph_rdi_write(PGRAPHState* pg, uint32_t select, uint32_t address, uint32_t val)
{
	switch (select)
	{
	case RDI_INDEX_VTX_CONSTANTS0:
	case RDI_INDEX_VTX_CONSTANTS1:
		...
		assert((address / 4) < NV2A_VERTEXSHADER_CONSTANTS);
		pg->vsh_constants_dirty[address / 4] |= (val != pg->vsh_constants[address / 4][3 - address % 4]);
...
uint64_t pgraph_read(void* opaque, hwaddr addr, uint32_t size)
{
	...
	{
		uint32_t select  = GET_MASK(pg->regs[NV_PGRAPH_RDI_INDEX], NV_PGRAPH_RDI_INDEX_SELECT);
		uint32_t address = GET_MASK(pg->regs[NV_PGRAPH_RDI_INDEX], NV_PGRAPH_RDI_INDEX_ADDRESS);

		r = pgraph_rdi_read(pg, select, address);

		// FIXME: Overflow into select?
		assert(address < GET_MASK(NV_PGRAPH_RDI_INDEX_ADDRESS, NV_PGRAPH_RDI_INDEX_ADDRESS));
		SET_MASK(pg->regs[NV_PGRAPH_RDI_INDEX], NV_PGRAPH_RDI_INDEX_ADDRESS, address + 1);
		break;
	}
	default:
		r = pg->regs[addr];
...
void pgraph_write(void* opaque, hwaddr addr, uint64_t val, uint32_t size)
{
	NV2AState*   d  = (NV2AState*)opaque;
	PGRAPHState* pg = &d->pgraph;
	...
	case NV_PGRAPH_CHANNEL_CTX_TRIGGER:
	{
		...
		{
#ifdef DEBUG_NV2A
			unsigned pgraph_channel_id = GET_MASK(pg->regs[NV_PGRAPH_CTX_USER], NV_PGRAPH_CTX_USER_CHID);
#endif
...
void pgraph_flush(NV2AState* d)
{
	...
	QTAILQ_FOREACH_SAFE(s, &d->pgraph.surfaces, entry, next) { pgraph_surface_invalidate(d, s); }
...
#define METHOD_NAME_STR(gclass, name)  tostring(gclass##_##name)
#define METHOD_FUNC_NAME(gclass, name) pgraph_##gclass##_##name##_handler
#define METHOD_HANDLER_ARG_DECL        NV2AState *d, PGRAPHState *pg, uint32_t subchannel, uint32_t method, uint32_t parameter, uint32_t *parameters, size_t num_words_available, size_t *num_words_consumed, bool inc
#define METHOD_HANDLER_ARGS            d, pg, subchannel, method, parameter, parameters, num_words_available, num_words_consumed, inc
#define DEF_METHOD_PROTO(gclass, name) static void METHOD_FUNC_NAME(gclass, name)(METHOD_HANDLER_ARG_DECL)

#define DEF_METHOD(gclass, name)                               DEF_METHOD_PROTO(gclass, name);
#define DEF_METHOD_RANGE(gclass, name, range)                  DEF_METHOD_PROTO(gclass, name);
#define DEF_METHOD_CASE_4_OFFSET(gclass, name, offset, stride) // Drop
#define DEF_METHOD_CASE_4(gclass, name, stride)                DEF_METHOD_PROTO(gclass, name);
...
#define DEF_METHOD_RANGE(gclass, name, range)                                                                                \
	[METHOD_ADDR_TO_INDEX(METHOD_ADDR(gclass, name))... METHOD_ADDR_TO_INDEX(METHOD_ADDR(gclass, name) + 4 * range - 1)] = { \
		METHOD_ADDR(gclass, name),                                                                                           \
		METHOD_NAME_STR(gclass, name),                                                                                       \
		METHOD_FUNC_NAME(gclass, name),                                                                                      \
	},
...
#define DEF_METHOD(gclass, name)                               static const size_t METHOD_RANGE_END_NAME(gclass, name) = METHOD_ADDR(gclass, name) + 4;
#define DEF_METHOD_RANGE(gclass, name, range)                  static const size_t METHOD_RANGE_END_NAME(gclass, name) = METHOD_ADDR(gclass, name) + 4 * range;
#define DEF_METHOD_CASE_4_OFFSET(gclass, name, offset, stride) // Drop
#define DEF_METHOD_CASE_4(gclass, name, stride)                static const size_t METHOD_RANGE_END_NAME(gclass, name) = METHOD_ADDR(gclass, name) + 4 * stride;
#include "pgraph_methods.h"
#undef DEF_METHOD
#undef DEF_METHOD_RANGE
#undef DEF_METHOD_CASE_4_OFFSET
...
static void pgraph_method_inc(MethodFunc handler, uint32_t end, METHOD_HANDLER_ARG_DECL)
{
	if (!inc)
	{
	...
	for (size_t i = 0; i < count; i++)
	{
		...
		if (i)
			pgraph_method_log(subchannel, NV_KELVIN_PRIMITIVE, method, parameter);
...
#define DEF_METHOD_INC(gclass, name)                                                                                     \
	...
	{                                                                                                                    \
		pgraph_method_inc(METHOD_FUNC_NAME_INT(gclass, name), METHOD_RANGE_END_NAME(gclass, name), METHOD_HANDLER_ARGS); \
	}                                                                                                                    \
	DEF_METHOD_INT(gclass, name)
...
#define DEF_METHOD_NON_INC(gclass, name)                                                \
	...
	DEF_METHOD(gclass, name)                                                            \
	{                                                                                   \
		pgraph_method_non_inc(METHOD_FUNC_NAME_INT(gclass, name), METHOD_HANDLER_ARGS); \
	}                                                                                   \
	DEF_METHOD_INT(gclass, name)
...
static void pgraph_image_blit(NV2AState* d)
{
	...
	uint8_t* source = (uint8_t*)nv_dma_map(d, context_surfaces->dma_image_source, &source_dma_len);
	assert(context_surfaces->source_offset < source_dma_len);
	source += context_surfaces->source_offset;

	uint8_t* dest = (uint8_t*)nv_dma_map(d, context_surfaces->dma_image_dest, &dest_dma_len);
	...
	if (surf_dest)
	{
		if (image_blit->height < surf_dest->height || image_blit->width < surf_dest->width)
			pgraph_download_surface_data_if_dirty(d, surf_dest);
		surf_dest->upload_pending = true;
	}

	hwaddr source_offset = image_blit->in_y * context_surfaces->source_pitch + image_blit->in_x * bytes_per_pixel;
	hwaddr dest_offset   = image_blit->out_y * context_surfaces->dest_pitch + image_blit->out_x * bytes_per_pixel;

	hwaddr source_size = (image_blit->height - 1) * context_surfaces->source_pitch + image_blit->width * bytes_per_pixel;
	...
	assert(source_addr + source_offset + source_size <= memory_region_size(d->vram));
	...
	if (image_blit->operation == NV09F_SET_OPERATION_SRCCOPY)
	{
		...
		for (uint32_t y = 0; y < image_blit->height; y++)
		{
	...
	else if (image_blit->operation == NV09F_SET_OPERATION_BLEND_AND)
	{
		...
		for (uint32_t y = 0; y < image_blit->height; y++)
		{
			for (uint32_t x = 0; x < image_blit->width; x++)
			{
	...
	memory_region_set_client_dirty(d->vram, dest_addr, dest_size, DIRTY_MEMORY_VGA);
	memory_region_set_client_dirty(d->vram, dest_addr, dest_size, DIRTY_MEMORY_NV2A_TEX);
}
...
int pgraph_method(NV2AState* d, uint32_t subchannel, uint32_t method, uint32_t parameter, uint32_t* parameters, size_t num_words_available, size_t max_lookahead_words, bool inc)
{
	...
	bool channel_valid = d->pgraph.regs[NV_PGRAPH_CTX_CONTROL] & NV_PGRAPH_CTX_CONTROL_CHID;
	...
	uint32_t graphics_class = GET_MASK(pg->regs[NV_PGRAPH_CTX_SWITCH1], NV_PGRAPH_CTX_SWITCH1_GRCLASS);
	...
	case NV_BETA:
	{
		...
		case NV012_SET_BETA:
			...
			{
				// The parameter is a signed fixed-point number with a sign bit and 31 fractional bits.
...
DEF_METHOD(NV097, NO_OPERATION)
{
	...
	SET_MASK(pg->regs[NV_PGRAPH_TRAPPED_ADDR], NV_PGRAPH_TRAPPED_ADDR_CHID, channel_id);
	SET_MASK(pg->regs[NV_PGRAPH_TRAPPED_ADDR], NV_PGRAPH_TRAPPED_ADDR_SUBCH, subchannel);
	SET_MASK(pg->regs[NV_PGRAPH_TRAPPED_ADDR], NV_PGRAPH_TRAPPED_ADDR_MTHD, method);
...
{
	SET_MASK(pg->regs[NV_PGRAPH_SURFACE], NV_PGRAPH_SURFACE_READ_3D, parameter);
}

DEF_METHOD(NV097, SET_FLIP_WRITE)
{
	SET_MASK(pg->regs[NV_PGRAPH_SURFACE], NV_PGRAPH_SURFACE_WRITE_3D, parameter);
}

DEF_METHOD(NV097, SET_FLIP_MODULO)
{
	SET_MASK(pg->regs[NV_PGRAPH_SURFACE], NV_PGRAPH_SURFACE_MODULO_3D, parameter);
}
...
DEF_METHOD(NV097, SET_SURFACE_CLIP_HORIZONTAL)
{
	...
	pg->surface_shape.clip_x     = GET_MASK(parameter, NV097_SET_SURFACE_CLIP_HORIZONTAL_X);
	pg->surface_shape.clip_width = GET_MASK(parameter, NV097_SET_SURFACE_CLIP_HORIZONTAL_WIDTH);
}
...
DEF_METHOD(NV097, SET_SURFACE_CLIP_VERTICAL)
{
	...
	pg->surface_shape.clip_y      = GET_MASK(parameter, NV097_SET_SURFACE_CLIP_VERTICAL_Y);
	pg->surface_shape.clip_height = GET_MASK(parameter, NV097_SET_SURFACE_CLIP_VERTICAL_HEIGHT);
}
...
DEF_METHOD(NV097, SET_SURFACE_FORMAT)
{
	...
	pg->surface_shape.color_format  = GET_MASK(parameter, NV097_SET_SURFACE_FORMAT_COLOR);
	pg->surface_shape.zeta_format   = GET_MASK(parameter, NV097_SET_SURFACE_FORMAT_ZETA);
	pg->surface_shape.anti_aliasing = GET_MASK(parameter, NV097_SET_SURFACE_FORMAT_ANTI_ALIASING);
	pg->surface_shape.log_width     = GET_MASK(parameter, NV097_SET_SURFACE_FORMAT_WIDTH);
	pg->surface_shape.log_height    = GET_MASK(parameter, NV097_SET_SURFACE_FORMAT_HEIGHT);
...
{
	pgraph_update_surface(d, false, true, true);
	uint32_t color_pitch = GET_MASK(parameter, NV097_SET_SURFACE_PITCH_COLOR);
	uint32_t zeta_pitch  = GET_MASK(parameter, NV097_SET_SURFACE_PITCH_ZETA);
...
DEF_METHOD(NV097, SET_CONTROL0)
{
	...
	bool stencil_write_enable = parameter & NV097_SET_CONTROL0_STENCIL_WRITE_ENABLE;
	SET_MASK(pg->regs[NV_PGRAPH_CONTROL_0], NV_PGRAPH_CONTROL_0_STENCIL_WRITE_ENABLE, stencil_write_enable);

	uint32_t z_format = GET_MASK(parameter, NV097_SET_CONTROL0_Z_FORMAT);
	SET_MASK(pg->regs[NV_PGRAPH_SETUPRASTER], NV_PGRAPH_SETUPRASTER_Z_FORMAT, z_format);

	bool z_perspective = parameter & NV097_SET_CONTROL0_Z_PERSPECTIVE_ENABLE;
	SET_MASK(pg->regs[NV_PGRAPH_CONTROL_0], NV_PGRAPH_CONTROL_0_Z_PERSPECTIVE_ENABLE, z_perspective);
}
...
DEF_METHOD(NV097, SET_FOG_MODE)
{
	...
	SET_MASK(pg->regs[NV_PGRAPH_CONTROL_3], NV_PGRAPH_CONTROL_3_FOG_MODE, mode);
}
...
DEF_METHOD(NV097, SET_FOG_ENABLE)
{
	...
	*/
	SET_MASK(pg->regs[NV_PGRAPH_CONTROL_3], NV_PGRAPH_CONTROL_3_FOGENABLE, parameter);
}
...
{
	SET_MASK(pg->regs[NV_PGRAPH_SETUPRASTER], NV_PGRAPH_SETUPRASTER_WINDOWCLIPTYPE, parameter);
}
...
{
	SET_MASK(pg->regs[NV_PGRAPH_CONTROL_0], NV_PGRAPH_CONTROL_0_ALPHATESTENABLE, parameter);
}
...
{
	SET_MASK(pg->regs[NV_PGRAPH_SETUPRASTER], NV_PGRAPH_SETUPRASTER_CULLENABLE, parameter);
}

DEF_METHOD(NV097, SET_DEPTH_TEST_ENABLE)
{
	SET_MASK(pg->regs[NV_PGRAPH_CONTROL_0], NV_PGRAPH_CONTROL_0_ZENABLE, parameter);
}

DEF_METHOD(NV097, SET_DITHER_ENABLE)
{
	SET_MASK(pg->regs[NV_PGRAPH_CONTROL_0], NV_PGRAPH_CONTROL_0_DITHERENABLE, parameter);
}

DEF_METHOD(NV097, SET_LIGHTING_ENABLE)
{
	SET_MASK(pg->regs[NV_PGRAPH_CSV0_C], NV_PGRAPH_CSV0_C_LIGHTING, parameter);
}

DEF_METHOD(NV097, SET_POINT_PARAMS_ENABLE)
{
	SET_MASK(pg->regs[NV_PGRAPH_CSV0_D], NV_PGRAPH_CSV0_D_POINTPARAMSENABLE, parameter);
	SET_MASK(pg->regs[NV_PGRAPH_CONTROL_3], NV_PGRAPH_CONTROL_3_POINTPARAMSENABLE, parameter);
}

DEF_METHOD(NV097, SET_POINT_SMOOTH_ENABLE)
{
	SET_MASK(pg->regs[NV_PGRAPH_SETUPRASTER], NV_PGRAPH_SETUPRASTER_POINTSMOOTHENABLE, parameter);
}

DEF_METHOD(NV097, SET_LINE_SMOOTH_ENABLE)
{
	SET_MASK(pg->regs[NV_PGRAPH_SETUPRASTER], NV_PGRAPH_SETUPRASTER_LINESMOOTHENABLE, parameter);
}

DEF_METHOD(NV097, SET_POLY_SMOOTH_ENABLE)
{
	SET_MASK(pg->regs[NV_PGRAPH_SETUPRASTER], NV_PGRAPH_SETUPRASTER_POLYSMOOTHENABLE, parameter);
}

DEF_METHOD(NV097, SET_SKIN_MODE)
{
	SET_MASK(pg->regs[NV_PGRAPH_CSV0_D], NV_PGRAPH_CSV0_D_SKIN, parameter);
}

DEF_METHOD(NV097, SET_STENCIL_TEST_ENABLE)
{
	SET_MASK(pg->regs[NV_PGRAPH_CONTROL_1], NV_PGRAPH_CONTROL_1_STENCIL_TEST_ENABLE, parameter);
}

DEF_METHOD(NV097, SET_POLY_OFFSET_POINT_ENABLE)
{
	SET_MASK(pg->regs[NV_PGRAPH_SETUPRASTER], NV_PGRAPH_SETUPRASTER_POFFSETPOINTENABLE, parameter);
}

DEF_METHOD(NV097, SET_POLY_OFFSET_LINE_ENABLE)
{
	SET_MASK(pg->regs[NV_PGRAPH_SETUPRASTER], NV_PGRAPH_SETUPRASTER_POFFSETLINEENABLE, parameter);
}

DEF_METHOD(NV097, SET_POLY_OFFSET_FILL_ENABLE)
{
	SET_MASK(pg->regs[NV_PGRAPH_SETUPRASTER], NV_PGRAPH_SETUPRASTER_POFFSETFILLENABLE, parameter);
}

DEF_METHOD(NV097, SET_ALPHA_FUNC)
{
	SET_MASK(pg->regs[NV_PGRAPH_CONTROL_0], NV_PGRAPH_CONTROL_0_ALPHAFUNC, parameter & 0xF);
}

DEF_METHOD(NV097, SET_ALPHA_REF)
{
	SET_MASK(pg->regs[NV_PGRAPH_CONTROL_0], NV_PGRAPH_CONTROL_0_ALPHAREF, parameter);
}
...
{
	SET_MASK(pg->regs[NV_PGRAPH_CONTROL_0], NV_PGRAPH_CONTROL_0_ZFUNC, parameter & 0xF);
}
...
DEF_METHOD(NV097, SET_COLOR_MASK)
{
	...
	SET_MASK(pg->regs[NV_PGRAPH_CONTROL_0], NV_PGRAPH_CONTROL_0_ALPHA_WRITE_ENABLE, alpha);
	SET_MASK(pg->regs[NV_PGRAPH_CONTROL_0], NV_PGRAPH_CONTROL_0_RED_WRITE_ENABLE, red);
	SET_MASK(pg->regs[NV_PGRAPH_CONTROL_0], NV_PGRAPH_CONTROL_0_GREEN_WRITE_ENABLE, green);
	SET_MASK(pg->regs[NV_PGRAPH_CONTROL_0], NV_PGRAPH_CONTROL_0_BLUE_WRITE_ENABLE, blue);
}
...
{
	SET_MASK(pg->regs[NV_PGRAPH_CONTROL_1], NV_PGRAPH_CONTROL_1_STENCIL_MASK_WRITE, parameter);
}

DEF_METHOD(NV097, SET_STENCIL_FUNC)
{
	SET_MASK(pg->regs[NV_PGRAPH_CONTROL_1], NV_PGRAPH_CONTROL_1_STENCIL_FUNC, parameter & 0xF);
}

DEF_METHOD(NV097, SET_STENCIL_FUNC_REF)
{
	SET_MASK(pg->regs[NV_PGRAPH_CONTROL_1], NV_PGRAPH_CONTROL_1_STENCIL_REF, parameter);
}

DEF_METHOD(NV097, SET_STENCIL_FUNC_MASK)
{
	SET_MASK(pg->regs[NV_PGRAPH_CONTROL_1], NV_PGRAPH_CONTROL_1_STENCIL_MASK_READ, parameter);
}

DEF_METHOD(NV097, SET_STENCIL_OP_FAIL)
{
	SET_MASK(pg->regs[NV_PGRAPH_CONTROL_2], NV_PGRAPH_CONTROL_2_STENCIL_OP_FAIL, kelvin_map_stencil_op(parameter));
}

DEF_METHOD(NV097, SET_STENCIL_OP_ZFAIL)
{
	SET_MASK(pg->regs[NV_PGRAPH_CONTROL_2], NV_PGRAPH_CONTROL_2_STENCIL_OP_ZFAIL, kelvin_map_stencil_op(parameter));
}

DEF_METHOD(NV097, SET_STENCIL_OP_ZPASS)
{
	SET_MASK(pg->regs[NV_PGRAPH_CONTROL_2], NV_PGRAPH_CONTROL_2_STENCIL_OP_ZPASS, kelvin_map_stencil_op(parameter));
}
...
{
	SET_MASK(pg->regs[NV_PGRAPH_SETUPRASTER], NV_PGRAPH_SETUPRASTER_FRONTFACEMODE, kelvin_map_polygon_mode(parameter));
}

DEF_METHOD(NV097, SET_BACK_POLYGON_MODE)
{
	SET_MASK(pg->regs[NV_PGRAPH_SETUPRASTER], NV_PGRAPH_SETUPRASTER_BACKFACEMODE, kelvin_map_polygon_mode(parameter));
}
...
DEF_METHOD(NV097, SET_CULL_FACE)
{
	...
	SET_MASK(pg->regs[NV_PGRAPH_SETUPRASTER], NV_PGRAPH_SETUPRASTER_CULLCTRL, face);
}
...
{
	SET_MASK(pg->regs[NV_PGRAPH_CSV0_C], NV_PGRAPH_CSV0_C_NORMALIZATION_ENABLE, parameter);
}
...
{
	SET_MASK(d->pgraph.regs[NV_PGRAPH_CSV0_D], NV_PGRAPH_CSV0_D_LIGHTS, parameter);
}

DEF_METHOD(NV097, SET_TEXGEN_S)
{
	int      slot = (method - NV097_SET_TEXGEN_S) / 16;
	uint32_t reg  = (slot < 2) ? NV_PGRAPH_CSV1_A : NV_PGRAPH_CSV1_B;
	uint32_t mask = (slot % 2) ? NV_PGRAPH_CSV1_A_T1_S : NV_PGRAPH_CSV1_A_T0_S;
...
DEF_METHOD(NV097, SET_TEXGEN_T)
{
	...
	uint32_t reg  = (slot < 2) ? NV_PGRAPH_CSV1_A : NV_PGRAPH_CSV1_B;
	uint32_t mask = (slot % 2) ? NV_PGRAPH_CSV1_A_T1_T : NV_PGRAPH_CSV1_A_T0_T;
...
DEF_METHOD(NV097, SET_TEXGEN_R)
{
	...
	uint32_t reg  = (slot < 2) ? NV_PGRAPH_CSV1_A : NV_PGRAPH_CSV1_B;
	uint32_t mask = (slot % 2) ? NV_PGRAPH_CSV1_A_T1_R : NV_PGRAPH_CSV1_A_T0_R;
...
DEF_METHOD(NV097, SET_TEXGEN_Q)
{
	...
	uint32_t reg  = (slot < 2) ? NV_PGRAPH_CSV1_A : NV_PGRAPH_CSV1_B;
	uint32_t mask = (slot % 2) ? NV_PGRAPH_CSV1_A_T1_Q : NV_PGRAPH_CSV1_A_T0_Q;
...
DEF_METHOD_INC(NV097, SET_PROJECTION_MATRIX)
{
	...
	uint32_t row                     = NV_IGRAPH_XF_XFCTX_PMAT0 + slot / 4;
...
DEF_METHOD_INC(NV097, SET_MODEL_VIEW_MATRIX)
{
	...
	uint32_t matnum                   = slot / 16;
	uint32_t entry                    = slot % 16;
	uint32_t row                      = NV_IGRAPH_XF_XFCTX_MMAT0 + matnum * 8 + entry / 4;
...
DEF_METHOD_INC(NV097, SET_INVERSE_MODEL_VIEW_MATRIX)
{
	...
	uint32_t matnum                   = slot / 16;
	uint32_t entry                    = slot % 16;
	uint32_t row                      = NV_IGRAPH_XF_XFCTX_IMMAT0 + matnum * 8 + entry / 4;
	pg->vsh_constants[row][entry % 4] = parameter;
	pg->vsh_constants_dirty[row]      = true;
}
...
DEF_METHOD_INC(NV097, SET_COMPOSITE_MATRIX)
{
	...
	uint32_t row                     = NV_IGRAPH_XF_XFCTX_CMAT0 + slot / 4;
	pg->vsh_constants[row][slot % 4] = parameter;
	pg->vsh_constants_dirty[row]     = true;
}
...
DEF_METHOD_INC(NV097, SET_TEXTURE_MATRIX)
{
	...
	uint32_t tex                      = slot / 16;
	uint32_t entry                    = slot % 16;
	uint32_t row                      = NV_IGRAPH_XF_XFCTX_T0MAT + tex * 8 + entry / 4;
	pg->vsh_constants[row][entry % 4] = parameter;
	pg->vsh_constants_dirty[row]      = true;
}
...
DEF_METHOD_INC(NV097, SET_TEXGEN_PLANE_S)
{
	...
	uint32_t tex                      = slot / 16;
	uint32_t entry                    = slot % 16;
	uint32_t row                      = NV_IGRAPH_XF_XFCTX_TG0MAT + tex * 8 + entry / 4;
	pg->vsh_constants[row][entry % 4] = parameter;
	pg->vsh_constants_dirty[row]      = true;
}
...
DEF_METHOD_INC(NV097, SET_TRANSFORM_PROGRAM)
{
	...
	int program_load = GET_MASK(pg->regs[NV_PGRAPH_CHEOPS_OFFSET], NV_PGRAPH_CHEOPS_OFFSET_PROG_LD_PTR);
	...
	if (slot % 4 == 3)
		SET_MASK(pg->regs[NV_PGRAPH_CHEOPS_OFFSET], NV_PGRAPH_CHEOPS_OFFSET_PROG_LD_PTR, program_load + 1);
}

DEF_METHOD_INC(NV097, SET_TRANSFORM_CONSTANT)
{
	int slot       = (method - NV097_SET_TRANSFORM_CONSTANT) / 4;
	int const_load = GET_MASK(pg->regs[NV_PGRAPH_CHEOPS_OFFSET], NV_PGRAPH_CHEOPS_OFFSET_CONST_LD_PTR);

	assert(const_load < NV2A_VERTEXSHADER_CONSTANTS);
	// VertexShaderConstant *constant = &pg->constants[const_load];
	pg->vsh_constants_dirty[const_load] |= (parameter != pg->vsh_constants[const_load][slot % 4]);
	pg->vsh_constants[const_load][slot % 4] = parameter;

	if (slot % 4 == 3)
		SET_MASK(pg->regs[NV_PGRAPH_CHEOPS_OFFSET], NV_PGRAPH_CHEOPS_OFFSET_CONST_LD_PTR, const_load + 1);
}

DEF_METHOD_INC(NV097, SET_VERTEX3F)
{
	int              slot      = (method - NV097_SET_VERTEX3F) / 4;
	VertexAttribute* attribute = &pg->vertex_attributes[NV2A_VERTEX_ATTR_POSITION];
...
DEF_METHOD_INC(NV097, SET_BACK_LIGHT_AMBIENT_COLOR)
{
	...
	uint32_t part = NV097_SET_BACK_LIGHT_AMBIENT_COLOR / 4 + slot % 16;
	slot /= 16; // [Light index]
	assert(slot < 8);
	switch (part * 4)
	{
	case NV097_SET_BACK_LIGHT_AMBIENT_COLOR ... NV097_SET_BACK_LIGHT_AMBIENT_COLOR + 8:
		...
		break;
	case NV097_SET_BACK_LIGHT_DIFFUSE_COLOR ... NV097_SET_BACK_LIGHT_DIFFUSE_COLOR + 8:
		...
		break;
	case NV097_SET_BACK_LIGHT_SPECULAR_COLOR ... NV097_SET_BACK_LIGHT_SPECULAR_COLOR + 8:
...
DEF_METHOD_INC(NV097, SET_LIGHT_AMBIENT_COLOR)
{
	...
	uint32_t part = NV097_SET_LIGHT_AMBIENT_COLOR / 4 + slot % 32;
	slot /= 32; // [Light index]
	assert(slot < 8);
	switch (part * 4)
	{
	case NV097_SET_LIGHT_AMBIENT_COLOR ... NV097_SET_LIGHT_AMBIENT_COLOR + 8:
		...
		break;
	case NV097_SET_LIGHT_DIFFUSE_COLOR ... NV097_SET_LIGHT_DIFFUSE_COLOR + 8:
		...
		break;
	case NV097_SET_LIGHT_SPECULAR_COLOR ... NV097_SET_LIGHT_SPECULAR_COLOR + 8:
	...
	case NV097_SET_LIGHT_LOCAL_RANGE:
		...
		break;
	case NV097_SET_LIGHT_INFINITE_HALF_VECTOR ... NV097_SET_LIGHT_INFINITE_HALF_VECTOR + 8:
		part -= NV097_SET_LIGHT_INFINITE_HALF_VECTOR / 4;
		pg->light_infinite_half_vector[slot][part] = *(float*)&parameter;
		break;
	case NV097_SET_LIGHT_INFINITE_DIRECTION ... NV097_SET_LIGHT_INFINITE_DIRECTION + 8:
		part -= NV097_SET_LIGHT_INFINITE_DIRECTION / 4;
		pg->light_infinite_direction[slot][part] = *(float*)&parameter;
		break;
	case NV097_SET_LIGHT_SPOT_FALLOFF ... NV097_SET_LIGHT_SPOT_FALLOFF + 8:
		...
		break;
	case NV097_SET_LIGHT_SPOT_DIRECTION ... NV097_SET_LIGHT_SPOT_DIRECTION + 12:
		...
		break;
	case NV097_SET_LIGHT_LOCAL_POSITION ... NV097_SET_LIGHT_LOCAL_POSITION + 8:
		part -= NV097_SET_LIGHT_LOCAL_POSITION / 4;
		pg->light_local_position[slot][part] = *(float*)&parameter;
		break;
	case NV097_SET_LIGHT_LOCAL_ATTENUATION ... NV097_SET_LIGHT_LOCAL_ATTENUATION + 8:
...
DEF_METHOD_INC(NV097, SET_NORMAL3S)
{
	...
	VertexAttribute* attribute = &pg->vertex_attributes[NV2A_VERTEX_ATTR_NORMAL];
...
{
	SET_VERTEX_ATTRIBUTE_F(NV097_SET_SPECULAR_COLOR4F, NV2A_VERTEX_ATTR_SPECULAR);
}
...
{
	SET_VERTEX_ATRIBUTE_TEX_2F(NV097_SET_TEXCOORD0_2F, NV2A_VERTEX_ATTR_TEXTURE0);
}

DEF_METHOD_INC(NV097, SET_TEXCOORD1_2F)
{
	SET_VERTEX_ATRIBUTE_TEX_2F(NV097_SET_TEXCOORD1_2F, NV2A_VERTEX_ATTR_TEXTURE1);
}

DEF_METHOD_INC(NV097, SET_TEXCOORD2_2F)
{
	SET_VERTEX_ATRIBUTE_TEX_2F(NV097_SET_TEXCOORD2_2F, NV2A_VERTEX_ATTR_TEXTURE0);
}

DEF_METHOD_INC(NV097, SET_TEXCOORD3_2F)
{
	SET_VERTEX_ATRIBUTE_TEX_2F(NV097_SET_TEXCOORD3_2F, NV2A_VERTEX_ATTR_TEXTURE1);
}
...
{
	SET_VERTEX_ATTRIBUTE_4UB(NV097_SET_DIFFUSE_COLOR4UB, NV2A_VERTEX_ATTR_DIFFUSE);
}

DEF_METHOD_INC(NV097, SET_SPECULAR_COLOR4UB)
{
	SET_VERTEX_ATTRIBUTE_4UB(NV097_SET_SPECULAR_COLOR4UB, NV2A_VERTEX_ATTR_SPECULAR);
}
...
DEF_METHOD_INC(NV097, SET_VERTEX_DATA_ARRAY_FORMAT)
{
	...
	attr->stride          = GET_MASK(parameter, NV097_SET_VERTEX_DATA_ARRAY_FORMAT_STRIDE);
...
{
	SET_MASK(pg->regs[NV_PGRAPH_BLEND], NV_PGRAPH_BLEND_LOGICOP_ENABLE, parameter);
}

DEF_METHOD(NV097, SET_LOGIC_OP)
{
	SET_MASK(pg->regs[NV_PGRAPH_BLEND], NV_PGRAPH_BLEND_LOGICOP, parameter & 0xF);
}
...
{
	bool depth_test   = pg->regs[NV_PGRAPH_CONTROL_0] & NV_PGRAPH_CONTROL_0_ZENABLE;
	bool stencil_test = pg->regs[NV_PGRAPH_CONTROL_1] & NV_PGRAPH_CONTROL_1_STENCIL_TEST_ENABLE;
	...
	if (parameter == NV097_SET_BEGIN_END_OP_END)
	{
		...
		int drawMode = pg->shader_binding->gl_primitive_mode;
		...
		if (pg->draw_arrays_length)
		{
			...
			pgraph_bind_vertex_attributes(d, pg->draw_arrays_min_start, pg->draw_arrays_max_count - 1, false, 0, pg->draw_arrays_max_count - 1);
		...
		else if (pg->inline_buffer_length)
		{
			...
			for (int i = 0; i < NV2A_VERTEXSHADER_ATTRIBUTES; i++)
			{
				...
				if (attr->inline_buffer_populated)
				{
					...
					memcpy(attr->inline_value, attr->inline_buffer + (pg->inline_buffer_length - 1) * 4, sizeof(attr->inline_value));
				}
				else
				{
			...
			glDrawArrays(drawMode, 0, pg->inline_buffer_length);
		}
		else if (pg->inline_array_length)
		{
			...
			uint32_t index_count = pgraph_bind_inline_array(d);
			glDrawArrays(drawMode, 0, index_count);
		}
		else if (pg->inline_elements_length)
		{
			...
			NewDrawBegin(drawMode);
			SetIndices(pg->inline_elements, pg->inline_elements_length, min_element, max_element);

			pgraph_bind_vertex_attributes(d, min_element, max_element, false, 0, pg->inline_elements[pg->inline_elements_length - 1]);
			...
			uint64_t h     = fast_hash((uint8_t*)pg->inline_elements, pg->inline_elements_length * 4);
	...
	else
	{
		...
		glStencilMask(GET_MASK(pg->regs[NV_PGRAPH_CONTROL_1], NV_PGRAPH_CONTROL_1_STENCIL_MASK_WRITE));

		if (pg->regs[NV_PGRAPH_BLEND] & NV_PGRAPH_BLEND_EN)
		{
			glEnable(GL_BLEND);
			uint32_t sfactor = GET_MASK(pg->regs[NV_PGRAPH_BLEND], NV_PGRAPH_BLEND_SFACTOR);
			uint32_t dfactor = GET_MASK(pg->regs[NV_PGRAPH_BLEND], NV_PGRAPH_BLEND_DFACTOR);
			assert(sfactor < ARRAY_SIZE(pgraph_blend_factor_map));
			assert(dfactor < ARRAY_SIZE(pgraph_blend_factor_map));
			glBlendFunc(pgraph_blend_factor_map[sfactor], pgraph_blend_factor_map[dfactor]);

			uint32_t equation = GET_MASK(pg->regs[NV_PGRAPH_BLEND], NV_PGRAPH_BLEND_EQN);
			...
			glBlendColor(
			    ((blend_color >> 16) & 0xFF) / 255.0f,  // red
		...
		// Face culling
		if (pg->regs[NV_PGRAPH_SETUPRASTER] & NV_PGRAPH_SETUPRASTER_CULLENABLE)
		{
			uint32_t cull_face = GET_MASK(pg->regs[NV_PGRAPH_SETUPRASTER], NV_PGRAPH_SETUPRASTER_CULLCTRL);
		...
		// Front-face select
		glFrontFace(pg->regs[NV_PGRAPH_SETUPRASTER] & NV_PGRAPH_SETUPRASTER_FRONTFACE ? GL_CCW : GL_CW);

		// Polygon offset
		// FIXME: GL implementation-specific, maybe do this in VS?
		if (pg->regs[NV_PGRAPH_SETUPRASTER] & NV_PGRAPH_SETUPRASTER_POFFSETFILLENABLE)
			glEnable(GL_POLYGON_OFFSET_FILL);
		else
			glDisable(GL_POLYGON_OFFSET_FILL);

		if (pg->regs[NV_PGRAPH_SETUPRASTER] & NV_PGRAPH_SETUPRASTER_POFFSETLINEENABLE)
			glEnable(GL_POLYGON_OFFSET_LINE);
		else
			glDisable(GL_POLYGON_OFFSET_LINE);

		if (pg->regs[NV_PGRAPH_SETUPRASTER] & NV_PGRAPH_SETUPRASTER_POFFSETPOINTENABLE)
		...
		if (depth_test)
		{
			...
			uint32_t depth_func = GET_MASK(pg->regs[NV_PGRAPH_CONTROL_0], NV_PGRAPH_CONTROL_0_ZFUNC);
		...
		if (stencil_test)
		{
			...
			uint32_t stencil_func = GET_MASK(pg->regs[NV_PGRAPH_CONTROL_1], NV_PGRAPH_CONTROL_1_STENCIL_FUNC);
			uint32_t stencil_ref  = GET_MASK(pg->regs[NV_PGRAPH_CONTROL_1], NV_PGRAPH_CONTROL_1_STENCIL_REF);
			uint32_t func_mask    = GET_MASK(pg->regs[NV_PGRAPH_CONTROL_1], NV_PGRAPH_CONTROL_1_STENCIL_MASK_READ);
			uint32_t op_fail      = GET_MASK(pg->regs[NV_PGRAPH_CONTROL_2], NV_PGRAPH_CONTROL_2_STENCIL_OP_FAIL);
			uint32_t op_zfail     = GET_MASK(pg->regs[NV_PGRAPH_CONTROL_2], NV_PGRAPH_CONTROL_2_STENCIL_OP_ZFAIL);
			uint32_t op_zpass     = GET_MASK(pg->regs[NV_PGRAPH_CONTROL_2], NV_PGRAPH_CONTROL_2_STENCIL_OP_ZPASS);
		...
		// Edge Antialiasing
		if (xsettings.line_smooth == 2)
		{
			if (pg->regs[NV_PGRAPH_SETUPRASTER] & NV_PGRAPH_SETUPRASTER_LINESMOOTHENABLE)
				glEnable(GL_LINE_SMOOTH);
			else
				glDisable(GL_LINE_SMOOTH);
		}
		else if (xsettings.line_smooth)
			glEnable(GL_LINE_SMOOTH);
		else
			glDisable(GL_LINE_SMOOTH);

		if (xsettings.polygon_smooth == 2)
		{
			if (pg->regs[NV_PGRAPH_SETUPRASTER] & NV_PGRAPH_SETUPRASTER_POLYSMOOTHENABLE)
				glEnable(GL_POLYGON_SMOOTH);
			else
				glDisable(GL_POLYGON_SMOOTH);
		}
		else if (xsettings.polygon_smooth)
			glEnable(GL_POLYGON_SMOOTH);
		else
			glDisable(GL_POLYGON_SMOOTH);
		...
		uint32_t vp_width = pg->surface_binding_dim.width, vp_height = pg->surface_binding_dim.height;
		pgraph_apply_scaling_factor(pg, &vp_width, &vp_height);
		glViewport(0, 0, vp_width, vp_height);

		// Surface clip
		// FIXME: Consider moving to PSH w/ window clip
		uint32_t xmin = pg->surface_shape.clip_x - pg->surface_binding_dim.clip_x;
		uint32_t ymin = pg->surface_shape.clip_y - pg->surface_binding_dim.clip_y;
		uint32_t xmax = xmin + pg->surface_shape.clip_width - 1;
		uint32_t ymax = ymin + pg->surface_shape.clip_height - 1;

		uint32_t scissor_width = xmax - xmin + 1, scissor_height = ymax - ymin + 1;
...
DEF_METHOD(NV097, SET_TEXTURE_FORMAT)
{
	...
	bool     dma_select     = GET_MASK(parameter, NV097_SET_TEXTURE_FORMAT_CONTEXT_DMA) == 2;
	bool     cubemap        = GET_MASK(parameter, NV097_SET_TEXTURE_FORMAT_CUBEMAP_ENABLE);
	uint32_t border_source  = GET_MASK(parameter, NV097_SET_TEXTURE_FORMAT_BORDER_SOURCE);
	uint32_t dimensionality = GET_MASK(parameter, NV097_SET_TEXTURE_FORMAT_DIMENSIONALITY);
	uint32_t color_format   = GET_MASK(parameter, NV097_SET_TEXTURE_FORMAT_COLOR);
	uint32_t levels         = GET_MASK(parameter, NV097_SET_TEXTURE_FORMAT_MIPMAP_LEVELS);
	uint32_t log_width      = GET_MASK(parameter, NV097_SET_TEXTURE_FORMAT_BASE_SIZE_U);
	uint32_t log_height     = GET_MASK(parameter, NV097_SET_TEXTURE_FORMAT_BASE_SIZE_V);
	uint32_t log_depth      = GET_MASK(parameter, NV097_SET_TEXTURE_FORMAT_BASE_SIZE_P);
...
DEF_METHOD(NV097, SET_TEXTURE_PALETTE)
{
	...
	bool     dma_select = GET_MASK(parameter, NV097_SET_TEXTURE_PALETTE_CONTEXT_DMA) == 1;
	uint32_t length     = GET_MASK(parameter, NV097_SET_TEXTURE_PALETTE_LENGTH);
	uint32_t offset     = GET_MASK(parameter, NV097_SET_TEXTURE_PALETTE_OFFSET);
...
DEF_METHOD(NV097, SET_TEXTURE_SET_BUMP_ENV_MAT)
{
	...
	slot -= 16;
	const int swizzle[4]                   = { NV_PGRAPH_BUMPMAT00, NV_PGRAPH_BUMPMAT01, NV_PGRAPH_BUMPMAT11, NV_PGRAPH_BUMPMAT10 };
...
{
	uint32_t start = GET_MASK(parameter, NV097_DRAW_ARRAYS_START_INDEX);
	uint32_t count = GET_MASK(parameter, NV097_DRAW_ARRAYS_COUNT) + 1;
	...
	{
		uint32_t last_start = pg->gl_draw_arrays_start[pg->draw_arrays_length - 1];
		GLsizei* last_count = &pg->gl_draw_arrays_count[pg->draw_arrays_length - 1];
...
DEF_METHOD(NV097, BACK_END_WRITE_SEMAPHORE_RELEASE)
{
	...
	uint8_t* semaphore_data = (uint8_t*)nv_dma_map(d, pg->dma_semaphore, &semaphore_dma_len);
...
DEF_METHOD(NV097, CLEAR_SURFACE)
{
	...
	if (write_color)
	{
		...
		glColorMask(
		    (parameter & NV097_CLEAR_SURFACE_R) ? GL_TRUE : GL_FALSE,
	...
	uint32_t xmin = GET_MASK(pg->regs[NV_PGRAPH_CLEARRECTX], NV_PGRAPH_CLEARRECTX_XMIN);
	uint32_t xmax = GET_MASK(pg->regs[NV_PGRAPH_CLEARRECTX], NV_PGRAPH_CLEARRECTX_XMAX);
	uint32_t ymin = GET_MASK(pg->regs[NV_PGRAPH_CLEARRECTY], NV_PGRAPH_CLEARRECTY_YMIN);
	uint32_t ymax = GET_MASK(pg->regs[NV_PGRAPH_CLEARRECTY], NV_PGRAPH_CLEARRECTY_YMAX);
	...
	bool full_clear = !xmin && !ymin && scissor_width >= pg->surface_binding_dim.width && scissor_height >= pg->surface_binding_dim.height;
	...
	if (pg->color_binding) pg->color_binding->cleared = full_clear && write_color;
	if (pg->zeta_binding) pg->zeta_binding->cleared = full_clear && write_zeta;
	pg->clearing = false;
}
...
{
	SET_MASK(pg->regs[NV_PGRAPH_SHADERCTL], 0xFFF, GET_MASK(parameter, 0xFFF));
}

DEF_METHOD(NV097, SET_SHADER_OTHER_STAGE_INPUT)
{
	SET_MASK(pg->regs[NV_PGRAPH_SHADERCTL], 0xFFFF000, GET_MASK(parameter, 0xFFFF000));
}
...
DEF_METHOD(NV097, SET_TRANSFORM_PROGRAM_LOAD)
{
	...
	SET_MASK(pg->regs[NV_PGRAPH_CHEOPS_OFFSET], NV_PGRAPH_CHEOPS_OFFSET_PROG_LD_PTR, parameter);
}
...
void pgraph_context_switch(NV2AState* d, uint32_t channel_id)
{
	...
	if (!valid)
	{
		SET_MASK(d->pgraph.regs[NV_PGRAPH_TRAPPED_ADDR], NV_PGRAPH_TRAPPED_ADDR_CHID, channel_id);

		NV2A_DPRINTF("pgraph switching to ch %d\n", channel_id);

		// TODO: hardware context switching
		assert(!(d->pgraph.regs[NV_PGRAPH_DEBUG_3] & NV_PGRAPH_DEBUG_3_HW_CONTEXT_SWITCH));
...
static void pgraph_finish_inline_buffer_vertex(PGRAPHState* pg)
{
	...
	for (int i = 0; i < NV2A_VERTEXSHADER_ATTRIBUTES; i++)
	{
		...
		if (attribute->inline_buffer_populated)
			memcpy(&attribute->inline_buffer[pg->inline_buffer_length * 4], attribute->inline_value, sizeof(float) * 4);
	}
...
void nv2a_set_surface_scale_factor(uint32_t scale)
{
	if (g_nv2a->pgraph.surface_scale_factor == scale)
		return;
...
uint32_t nv2a_get_surface_scale_factor(void)
{
...
static void pgraph_reload_surface_scale_factor(NV2AState* d)
{
	d->pgraph.surface_scale_factor = xsettings.resolution_scale;
}
...
void pgraph_init(NV2AState* d)
{
	...
	for (int i = 0; i < NV2A_VERTEXSHADER_ATTRIBUTES; i++)
	{
		...
		attribute->inline_buffer           = (float*)g_malloc(NV2A_MAX_BATCH_LENGTH * sizeof(float) * 4);
	...
	glBufferData(GL_ARRAY_BUFFER, memory_region_size(d->vram), NULL, GL_DYNAMIC_DRAW);
...
static void pgraph_shader_update_constants(PGRAPHState* pg, ShaderBinding* binding, bool binding_changed, bool vertex_program, bool fixed_function)
{
	...
	{
		float alpha_ref = GET_MASK(pg->regs[NV_PGRAPH_CONTROL_0], NV_PGRAPH_CONTROL_0_ALPHAREF) / 255.0;
	...
	for (int i = 0; i < NV2A_MAX_TEXTURES; i++)
	{
		...
		if (i > 0)
		{
			...
			if (loc != -1)
				glUniform1f(loc, *(float*)&pg->regs[NV_PGRAPH_BUMPSCALE1 + (i - 1) * 4]);

			loc = binding->bump_offset_loc[i];
			if (loc != -1)
				glUniform1f(loc, *(float*)&pg->regs[NV_PGRAPH_BUMPOFFSET1 + (i - 1) * 4]);
		}
	...
	if (fixed_function)
	{
		...
		uint32_t aa_width = 1, aa_height = 1;
		...
		if (binding->inv_viewport_loc != -1)
			glUniformMatrix4fv(binding->inv_viewport_loc, 1, GL_FALSE, &invViewport[0]);
	}
	...
	for (int i = 0; i < NV2A_VERTEXSHADER_CONSTANTS; i++)
	{
		...
		if ((loc != -1) && memcmp(binding->vsh_constants[i], pg->vsh_constants[i], sizeof(pg->vsh_constants[1])))
		{
			glUniform4fv(loc, 1, (const GLfloat*)pg->vsh_constants[i]);
			memcpy(binding->vsh_constants[i], pg->vsh_constants[i], sizeof(pg->vsh_constants[i]));
		}
	...
	for (int i = 0; i < 8; i++)
	{
		...
		uint32_t x_min = GET_MASK(x, NV_PGRAPH_WINDOWCLIPX0_XMIN);
		uint32_t x_max = GET_MASK(x, NV_PGRAPH_WINDOWCLIPX0_XMAX);
		uint32_t y     = pg->regs[NV_PGRAPH_WINDOWCLIPY0 + i * 4];
		uint32_t y_min = GET_MASK(y, NV_PGRAPH_WINDOWCLIPY0_YMIN);
		uint32_t y_max = GET_MASK(y, NV_PGRAPH_WINDOWCLIPY0_YMAX);
		...
		uint32_t y_min_xlat = MAX(pg->surface_binding_dim.height - y_max - 1, 0);
		uint32_t y_max_xlat = MIN(pg->surface_binding_dim.height - y_min - 1, pg->surface_binding_dim.height);
...
#define CR_x__check(reg, x)                         \
	for (int i = 0; i < x; i++)                     \
	{                                               \
		if (pg->regs[reg + i * 4] != CNAME(reg)[i]) \
			goto dirty;                             \
	}
#define CF_x__check(type, src, name, x) \
	for (int i = 0; i < x; i++)         \
	{                                   \
		if (src[i] != CNAME(name)[i])   \
			goto dirty;                 \
	}
...
static void pgraph_bind_shaders(PGRAPHState* pg)
{
	...
	state.psh.window_clip_exclusive = pg->regs[NV_PGRAPH_SETUPRASTER] & NV_PGRAPH_SETUPRASTER_WINDOWCLIPTYPE;
	...
	state.psh.alpha_test = pg->regs[NV_PGRAPH_CONTROL_0] & NV_PGRAPH_CONTROL_0_ALPHATESTENABLE;
	state.psh.alpha_func = (enum PshAlphaFunc)GET_MASK(pg->regs[NV_PGRAPH_CONTROL_0], NV_PGRAPH_CONTROL_0_ALPHAFUNC);

	state.psh.point_sprite = pg->regs[NV_PGRAPH_SETUPRASTER] & NV_PGRAPH_SETUPRASTER_POINTSMOOTHENABLE;
	...
	if (fixed_function)
	{
		state.skinning      = (enum VshSkinning)GET_MASK(pg->regs[NV_PGRAPH_CSV0_D], NV_PGRAPH_CSV0_D_SKIN);
		state.lighting      = GET_MASK(pg->regs[NV_PGRAPH_CSV0_C], NV_PGRAPH_CSV0_C_LIGHTING);
	...
	state.z_perspective  = pg->regs[NV_PGRAPH_CONTROL_0] & NV_PGRAPH_CONTROL_0_Z_PERSPECTIVE_ENABLE;

	state.point_params_enable = GET_MASK(pg->regs[NV_PGRAPH_CSV0_D], NV_PGRAPH_CSV0_D_POINTPARAMSENABLE);
	state.point_size          = GET_MASK(pg->regs[NV_PGRAPH_POINTSIZE], NV097_SET_POINT_SIZE_V) / 8.0f;
	...
	for (int i = 0; i < 4; i++)
	{
		uint32_t reg = (i < 2) ? NV_PGRAPH_CSV1_A : NV_PGRAPH_CSV1_B;
		for (int j = 0; j < 4; j++)
		{
			uint32_t masks[] = {
	...
	// Fog
	state.fog_enable = pg->regs[NV_PGRAPH_CONTROL_3] & NV_PGRAPH_CONTROL_3_FOGENABLE;
	if (state.fog_enable)
	{
		// FIXME: Use CSV0_D?
		state.fog_mode = (enum VshFogMode)GET_MASK(pg->regs[NV_PGRAPH_CONTROL_3], NV_PGRAPH_CONTROL_3_FOG_MODE);
		state.foggen   = (enum VshFoggen)GET_MASK(pg->regs[NV_PGRAPH_CSV0_D], NV_PGRAPH_CSV0_D_FOGGENMODE);
	}
	else
	{
	...
	for (int i = 0; i < 4; i++)
	{
		...
		bool enabled = pg->regs[NV_PGRAPH_TEXCTL0_0 + i * 4] & NV_PGRAPH_TEXCTL0_0_ENABLE;
		if (!enabled)
			continue;

		uint32_t        color_format = GET_MASK(pg->regs[NV_PGRAPH_TEXFMT0 + i * 4], NV_PGRAPH_TEXFMT0_COLOR);
		 ...
		 */
		state.psh.snorm_tex[i] = (f.gl_internal_format == GL_RGB8_SNORM) || (f.gl_internal_format == GL_RG8_SNORM);

		uint32_t               filter     = pg->regs[NV_PGRAPH_TEXFILTER0 + i * 4];
		uint32_t               min_filter = GET_MASK(filter, NV_PGRAPH_TEXFILTER0_MIN);
	...
	else
	{
		...
		g_hash_table_insert(pg->shader_cache, cache_state, (gpointer)pg->shader_binding);
	}
...
update_constants:
	pgraph_shader_update_constants(pg, pg->shader_binding, binding_changed, vertex_program, fixed_function);
...
static bool pgraph_framebuffer_dirty(PGRAPHState* pg)
{
	bool shape_changed = memcmp(&pg->surface_shape, &pg->last_surface_shape, sizeof(SurfaceShape)) != 0;
	if (!shape_changed || (!pg->surface_shape.color_format && !pg->surface_shape.zeta_format))
		return false;
	return true;
}
...
static GLuint pgraph_compile_shader(const char* vs_src, const char* fs_src)
{
	...
	if (xsettings.shader_hint)
		xemu_queue_notification("pgraph_compile_shader", true);
	glLinkProgram(prog);
	glUseProgram(prog);
...
static void pgraph_init_render_to_texture(NV2AState* d)
{
	...
	pg->s2t_rndr.surface_size_loc = glGetUniformLocation(pg->s2t_rndr.prog, "surface_size");
...
static bool pgraph_surface_to_texture_can_fastpath(SurfaceBinding* surface, TextureShape* shape)
{
...
static void pgraph_render_surface_to(NV2AState* d, SurfaceBinding* surface, int texture_unit, GLuint gl_target, GLuint gl_texture, uint32_t width, uint32_t height)
{
	glActiveTexture(GL_TEXTURE0 + texture_unit);
	glBindFramebuffer(GL_FRAMEBUFFER, d->pgraph.s2t_rndr.fbo);

	GLenum draw_buffers[1] = { GL_COLOR_ATTACHMENT0 };
	glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, gl_target, gl_texture, 0);
	...
	glProgramUniform1i(d->pgraph.s2t_rndr.prog, d->pgraph.s2t_rndr.tex_loc, texture_unit);
	glProgramUniform2f(d->pgraph.s2t_rndr.prog, d->pgraph.s2t_rndr.surface_size_loc, width, height);
	...
	glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, gl_target, 0, 0);
...
static void pgraph_render_surface_to_texture_slow(NV2AState* d, SurfaceBinding* surface, TextureBinding* texture, TextureShape* texture_shape, int texture_unit)
{
	...
	uint32_t width = surface->width, height = surface->height;
	...
	glTexImage2D(texture->gl_target, 0, f->gl_internal_format, width, height, 0, f->gl_format, f->gl_type, buf);
	g_free(buf);
	glBindTexture(texture->gl_target, texture->gl_texture);
}
...
static void pgraph_render_surface_to_texture(NV2AState* d, SurfaceBinding* surface, TextureBinding* texture, TextureShape* texture_shape, int texture_unit)
{
	...
	{
		pgraph_render_surface_to_texture_slow(d, surface, texture, texture_shape, texture_unit);
		return;
	}

	uint32_t width = texture_shape->width, height = texture_shape->height;
	pgraph_apply_scaling_factor(pg, &width, &height);
...
static uint8_t* convert_texture_data__CR8YB8CB8YA8(const uint8_t* data, uint32_t width, uint32_t height, uint32_t pitch)
{
...
static void pgraph_render_display_pvideo_overlay(NV2AState* d)
{
	...
	int in_width  = GET_MASK(d->pvideo.regs[NV_PVIDEO_SIZE_IN], NV_PVIDEO_SIZE_IN_WIDTH);
	int in_height = GET_MASK(d->pvideo.regs[NV_PVIDEO_SIZE_IN], NV_PVIDEO_SIZE_IN_HEIGHT);

	/* FIXME
	int in_s = GET_MASK(d->pvideo.regs[NV_PVIDEO_POINT_IN], NV_PVIDEO_POINT_IN_S);
	int in_t = GET_MASK(d->pvideo.regs[NV_PVIDEO_POINT_IN], NV_PVIDEO_POINT_IN_T);
	*/
	int in_pitch = GET_MASK(d->pvideo.regs[NV_PVIDEO_FORMAT], NV_PVIDEO_FORMAT_PITCH);
	int in_color = GET_MASK(d->pvideo.regs[NV_PVIDEO_FORMAT], NV_PVIDEO_FORMAT_COLOR);
	...
	uint32_t out_width  = GET_MASK(d->pvideo.regs[NV_PVIDEO_SIZE_OUT], NV_PVIDEO_SIZE_OUT_WIDTH);
	uint32_t out_height = GET_MASK(d->pvideo.regs[NV_PVIDEO_SIZE_OUT], NV_PVIDEO_SIZE_OUT_HEIGHT);
	...
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, xsettings.overlay_nearest ? GL_NEAREST : GL_LINEAR);
	uint8_t* tex_rgba = convert_texture_data__CR8YB8CB8YA8(d->vram_ptr + base + offset, in_width, in_height, in_pitch);
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, in_width, in_height, 0, GL_RGBA, GL_UNSIGNED_BYTE, tex_rgba);
	g_free(tex_rgba);
	glUniform1i(d->pgraph.disp_rndr.pvideo_tex_loc, 1);
	glUniform4f(d->pgraph.disp_rndr.pvideo_pos_loc, out_x, out_y, out_width, out_height);
}
...
static void pgraph_render_display(NV2AState* d, SurfaceBinding* surface)
{
	...
	bool recreate =
	    (surface->fmt.gl_internal_format != pg->gl_display_buffer_internal_format
	     || width != pg->gl_display_buffer_width || height != pg->gl_display_buffer_height
	...
	if (recreate)
	{
		 ...
		 * Track:
		 * https://community.intel.com/t5/Graphics/OpenGL-Windows-drivers-for-Intel-HD-630-leaking-GPU-memory-when/td-p/1274423
		 */
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_BASE_LEVEL, 0);
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAX_LEVEL, 0);
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, xsettings.display_nearest ? GL_NEAREST : GL_LINEAR);
...
static SurfaceBinding* pgraph_surface_get_within(NV2AState* d, hwaddr addr)
{
	...
	QTAILQ_FOREACH(surface, &d->pgraph.surfaces, entry)
	{
		if (addr >= surface->vram_addr && addr < (surface->vram_addr + surface->size))
			return surface;
	}
...
static void surface_copy_shrink_row(uint8_t* out, uint8_t* in, uint32_t width, uint32_t bytes_per_pixel, uint32_t factor)
{
...
static void pgraph_download_surface_data_to_buffer(NV2AState* d, SurfaceBinding* surface, bool swizzle, bool flip, bool downscale, uint8_t* pixels)
{
	...
	if (downscale)
	{
		...
		uint32_t inPitch = surface->pitch * pg->surface_scale_factor * pg->surface_scale_factor;
		for (uint32_t y = 0; y < surface->height; y++)
		{
			surface_copy_shrink_row(out, in, surface->width, surface->fmt.bytes_per_pixel, pg->surface_scale_factor);
			in += inPitch;
			out += surface->pitch;
		}
	}
...
{
	SurfaceBinding* surface;
	QTAILQ_FOREACH(surface, &d->pgraph.surfaces, entry) { pgraph_download_surface_data(d, surface, false); }
...
{
	SurfaceBinding* surface;
	QTAILQ_FOREACH(surface, &d->pgraph.surfaces, entry) { pgraph_download_surface_data_if_dirty(d, surface); }

	qatomic_set(&d->pgraph.download_dirty_surfaces_pending, false);
	qemu_event_set(&d->pgraph.dirty_surfaces_download_complete);
}

static void surface_copy_expand_row(uint8_t* out, uint8_t* in, uint32_t width, uint32_t bytes_per_pixel, uint32_t factor)
{
	if (bytes_per_pixel == 4)
	{
		for (uint32_t x = 0; x < width; x++)
...
static void surface_copy_expand(uint8_t* out, uint8_t* in, uint32_t width, uint32_t height, uint32_t bytes_per_pixel, uint32_t factor)
{
...
static void pgraph_populate_surface_binding_entry_sized(NV2AState* d, bool color, uint32_t width, uint32_t height, SurfaceBinding* entry)
{
	PGRAPHState*      pg = &d->pgraph;
	Surface*          surface;
	hwaddr            dma_address;
	...
	if (color)
	{
		...
		assert(pg->surface_shape.color_format < ARRAY_SIZE(kelvin_surface_color_format_map));
...
static void pgraph_update_surface_part(NV2AState* d, bool upload, bool color)
{
	...
	if (upload && (surface->buffer_dirty || mem_dirty))
	{
		...
		if (found != NULL)
		{
			 ...
			 */
			SurfaceBinding* other = (color ? pg->zeta_binding : pg->color_binding);
			if (found == other)
			{
		...
		if (found != NULL)
		{
			...
			if (is_compatible && color && !pgraph_check_surface_compatibility(found, &entry, true))
			{
				...
				pgraph_populate_surface_binding_entry_sized(d, !color, found->width, found->height, &zeta_entry);
				hwaddr color_end = found->vram_addr + found->size;
				hwaddr zeta_end  = zeta_entry.vram_addr + zeta_entry.size;
				is_compatible &= found->vram_addr >= zeta_end || zeta_entry.vram_addr >= color_end;
			}
		...
		if (should_create)
		{
			...
			uint32_t width = entry.width, height = entry.height;
			pgraph_apply_scaling_factor(pg, &width, &height);
		...
		glFramebufferTexture2D(GL_FRAMEBUFFER, entry.fmt.gl_attachment, GL_TEXTURE_2D, found->gl_buffer, 0);
		assert(glCheckFramebufferStatus(GL_FRAMEBUFFER) == GL_FRAMEBUFFER_COMPLETE);
	...
	if (!upload && surface->draw_dirty)
	{
		if (!tcg_enabled())
		{
			...
			pgraph_download_surface_data(d, color ? pg->color_binding : pg->zeta_binding, true);
		}
...
static void pgraph_update_surface(NV2AState* d, bool upload, bool color_write, bool zeta_write)
{
	...
	pg->surface_shape.z_format = GET_MASK(pg->regs[NV_PGRAPH_SETUPRASTER], NV_PGRAPH_SETUPRASTER_Z_FORMAT);
	...
	{
		if ((color_write || pg->surface_color.write_enabled_cache) && pg->surface_color.draw_dirty)
			pgraph_update_surface_part(d, false, true);
		if ((zeta_write || pg->surface_zeta.write_enabled_cache) && pg->surface_zeta.draw_dirty)
...
{
	struct pgraph_texture_possibly_dirty_struct* test = (struct pgraph_texture_possibly_dirty_struct*)opaque;
...
static void pgraph_mark_textures_possibly_dirty(NV2AState* d, hwaddr addr, hwaddr size)
{
	...
	lru_visit_active(&d->pgraph.texture_cache, pgraph_mark_textures_possibly_dirty_visitor, &test);
}
...
static bool pgraph_check_texture_dirty(NV2AState* d, hwaddr addr, hwaddr size)
{
	...
	addr &= TARGET_PAGE_MASK;
	assert(end < memory_region_size(d->vram));
	return memory_region_test_and_clear_dirty(d->vram, addr, end - addr, DIRTY_MEMORY_NV2A_TEX);
}
...
static void pgraph_bind_textures(NV2AState* d)
{
	...
	for (int i = 0; i < NV2A_MAX_TEXTURES; i++)
	{
		...
		uint32_t min_mipmap_level = GET_MASK(ctl_0, NV_PGRAPH_TEXCTL0_0_MIN_LOD_CLAMP);
		uint32_t max_mipmap_level = GET_MASK(ctl_0, NV_PGRAPH_TEXCTL0_0_MAX_LOD_CLAMP);

		uint32_t pitch = GET_MASK(ctl_1, NV_PGRAPH_TEXCTL1_0_IMAGE_PITCH);

		uint32_t dma_select     = GET_MASK(fmt, NV_PGRAPH_TEXFMT0_CONTEXT_DMA);
		bool     cubemap        = GET_MASK(fmt, NV_PGRAPH_TEXFMT0_CUBEMAPENABLE);
		uint32_t dimensionality = GET_MASK(fmt, NV_PGRAPH_TEXFMT0_DIMENSIONALITY);
		uint32_t color_format   = GET_MASK(fmt, NV_PGRAPH_TEXFMT0_COLOR);
		uint32_t levels         = GET_MASK(fmt, NV_PGRAPH_TEXFMT0_MIPMAP_LEVELS);
		uint32_t log_width      = GET_MASK(fmt, NV_PGRAPH_TEXFMT0_BASE_SIZE_U);
		uint32_t log_height     = GET_MASK(fmt, NV_PGRAPH_TEXFMT0_BASE_SIZE_V);
		uint32_t log_depth      = GET_MASK(fmt, NV_PGRAPH_TEXFMT0_BASE_SIZE_P);
...
#ifdef DEBUG_NV2A
		uint32_t lod_bias = GET_MASK(filter, NV_PGRAPH_TEXFILTER0_MIPMAP_LOD_BIAS);
#endif
		uint32_t min_filter = GET_MASK(filter, NV_PGRAPH_TEXFILTER0_MIN);
		uint32_t mag_filter = GET_MASK(filter, NV_PGRAPH_TEXFILTER0_MAG);

		uint32_t addru = GET_MASK(address, NV_PGRAPH_TEXADDRESS0_ADDRU);
		uint32_t addrv = GET_MASK(address, NV_PGRAPH_TEXADDRESS0_ADDRV);
		uint32_t addrp = GET_MASK(address, NV_PGRAPH_TEXADDRESS0_ADDRP);

		uint32_t border_source = GET_MASK(fmt, NV_PGRAPH_TEXFMT0_BORDER_SOURCE);
		uint32_t border_color  = pg->regs[NV_PGRAPH_BORDERCOLOR0 + i * 4];

		hwaddr offset = pg->regs[NV_PGRAPH_TEXOFFSET0 + i * 4];

		bool     palette_dma_select   = GET_MASK(palette, NV_PGRAPH_TEXPALETTE0_CONTEXT_DMA);
		uint32_t palette_length_index = GET_MASK(palette, NV_PGRAPH_TEXPALETTE0_LENGTH);
		uint32_t palette_offset       = palette & NV_PGRAPH_TEXPALETTE0_OFFSET;
		...
		{
			glBindTexture(pg->texture_binding[i]->gl_target, pg->texture_binding[i]->gl_texture);
			continue;
		}
		...
		assert((palette_vram_offset + palette_length) < memory_region_size(d->vram));
		...
		if (surface != NULL)
			surf_to_tex = pgraph_check_surface_to_texture_compatibility(surface, &state);
		...
		if (!surf_to_tex)
		{
			...
			QTAILQ_FOREACH(surface, &d->pgraph.surfaces, entry)
			{
				...
				bool   overlapping   = !(surface->vram_addr >= tex_vram_end || texture_vram_offset >= surf_vram_end);
				if (overlapping)
					pgraph_download_surface_data_if_dirty(d, surface);
			}
		}

		bool is_indexed = (color_format == NV097_SET_TEXTURE_FORMAT_COLOR_SZ_I8_A8R8G8B8);
		...
		LruNode*        found            = lru_lookup(&pg->texture_cache, tex_binding_hash, &key);
		TextureLruNode* key_out          = container_of(found, TextureLruNode, node);
		bool            possibly_dirty   = (key_out->binding == NULL) || key_out->possibly_dirty;
		...
		if (!surf_to_tex)
		{
			...
			if (is_indexed && pgraph_check_texture_dirty(d, palette_vram_offset, palette_length))
			{
				possibly_dirty = true;
				pgraph_mark_textures_possibly_dirty(d, palette_vram_offset, palette_length);
			}
		}
		...
		else
		{
			...
			glBindTexture(key_out->binding->gl_target, key_out->binding->gl_texture);
		}
		...
		if (xsettings.anisotropic > 0)
			glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MAX_ANISOTROPY_EXT, xsettings.anisotropic * 1.0f);

		// Texture wrapping
		assert(addru < ARRAY_SIZE(pgraph_texture_addr_map));
		glTexParameteri(binding->gl_target, GL_TEXTURE_WRAP_S, pgraph_texture_addr_map[addru]);
		if (dimensionality > 1)
		{
			assert(addrv < ARRAY_SIZE(pgraph_texture_addr_map));
			glTexParameteri(binding->gl_target, GL_TEXTURE_WRAP_T, pgraph_texture_addr_map[addrv]);
		}
		if (dimensionality > 2)
		{
			assert(addrp < ARRAY_SIZE(pgraph_texture_addr_map));
			glTexParameteri(binding->gl_target, GL_TEXTURE_WRAP_R, pgraph_texture_addr_map[addrp]);
		}
		...
		if (border_source == NV_PGRAPH_TEXFMT0_BORDER_SOURCE_COLOR)
		{
			...
			glTexParameterfv(binding->gl_target, GL_TEXTURE_BORDER_COLOR, gl_border_color);
		}
...
static void pgraph_update_memory_buffer(NV2AState* d, hwaddr addr, hwaddr size, bool quick)
{
	...
	size = end - addr;
	if (memory_region_test_and_clear_dirty(d->vram, addr, size, DIRTY_MEMORY_NV2A))
	{
...
static void pgraph_update_inline_value(VertexAttribute *attr, const uint8_t *data)
{
	assert(attr->count <= 4);
...
static void pgraph_bind_vertex_attributes(NV2AState* d, uint32_t min_element, uint32_t max_element, bool inline_data, uint32_t inline_stride, uint32_t provoking_element)
{
	...
	uint32_t     num_elements          = max_element - min_element + 1;

	if (inline_data)
		NV2A_GL_DGROUP_BEGIN("%s (num_elements: %d inline stride: %d)", __func__, num_elements, inline_stride);
	else
	...
	for (int i = 0; i < NV2A_VERTEXSHADER_ATTRIBUTES; i++)
	{
		...
		else
		{
			...
			pgraph_update_memory_buffer(d, start, num_elements * stride, updated_memory_buffer);
			updated_memory_buffer = true;

			// AddVertexAttribute(stride);
		}
...
static uint32_t pgraph_bind_inline_array(NV2AState* d)
{
	...
	uint32_t vertex_size = offset;
	uint32_t index_count = pg->inline_array_length * 4 / vertex_size;
	...
	glBindBuffer(GL_ARRAY_BUFFER, pg->gl_inline_array_buffer);
	glBufferData(GL_ARRAY_BUFFER, NV2A_MAX_BATCH_LENGTH * sizeof(uint32_t), NULL, GL_STREAM_DRAW);
	glBufferSubData(GL_ARRAY_BUFFER, 0, pg->inline_array_length * 4, pg->inline_array);
	pgraph_bind_vertex_attributes(d, 0, index_count - 1, true, vertex_size, index_count - 1);
...
static void upload_gl_texture(GLenum gl_target, const TextureShape s, const uint8_t* texture_data, const uint8_t* palette_data)
{
	...
	case GL_TEXTURE_CUBE_MAP_NEGATIVE_Z:
	{
		...
		for (int level = 0; level < s.levels; level++)
		{
			...
			else
			{
				...
				uint32_t pitch      = width * f.bytes_per_pixel;
	...
	case GL_TEXTURE_3D:
	{
		uint32_t width = s.width, height = s.height, depth = s.depth;
		...
		for (int level = 0; level < s.levels; level++)
		{
			...
			{
				uint32_t row_pitch   = width * f.bytes_per_pixel;
...
static TextureBinding* generate_texture(const TextureShape s, const uint8_t* texture_data, const uint8_t* palette_data)
{
	...
	if (gl_target == GL_TEXTURE_CUBE_MAP)
	{
		...
		upload_gl_texture(GL_TEXTURE_CUBE_MAP_POSITIVE_X, s, texture_data + 0 * length, palette_data);
		upload_gl_texture(GL_TEXTURE_CUBE_MAP_NEGATIVE_X, s, texture_data + 1 * length, palette_data);
		upload_gl_texture(GL_TEXTURE_CUBE_MAP_POSITIVE_Y, s, texture_data + 2 * length, palette_data);
		upload_gl_texture(GL_TEXTURE_CUBE_MAP_NEGATIVE_Y, s, texture_data + 3 * length, palette_data);
		upload_gl_texture(GL_TEXTURE_CUBE_MAP_POSITIVE_Z, s, texture_data + 4 * length, palette_data);
		upload_gl_texture(GL_TEXTURE_CUBE_MAP_NEGATIVE_Z, s, texture_data + 5 * length, palette_data);
	}
	else
	...
	if (!f.linear)
	{
		glTexParameteri(gl_target, GL_TEXTURE_BASE_LEVEL, s.min_mipmap_level);
		glTexParameteri(gl_target, GL_TEXTURE_MAX_LEVEL, s.levels - 1);
	}

	if (f.gl_swizzle_mask[0] != 0 || f.gl_swizzle_mask[1] != 0 || f.gl_swizzle_mask[2] != 0 || f.gl_swizzle_mask[3] != 0)
		glTexParameteriv(gl_target, GL_TEXTURE_SWIZZLE_RGBA, (const GLint*)f.gl_swizzle_mask);
...
static uint32_t kelvin_map_stencil_op(uint32_t parameter)
{
	...
	{
	// clang-format off
	case NV097_SET_STENCIL_OP_V_KEEP:    op = NV_PGRAPH_CONTROL_2_STENCIL_OP_V_KEEP; break;
	case NV097_SET_STENCIL_OP_V_ZERO:    op = NV_PGRAPH_CONTROL_2_STENCIL_OP_V_ZERO; break;
	case NV097_SET_STENCIL_OP_V_REPLACE: op = NV_PGRAPH_CONTROL_2_STENCIL_OP_V_REPLACE; break;
	case NV097_SET_STENCIL_OP_V_INCRSAT: op = NV_PGRAPH_CONTROL_2_STENCIL_OP_V_INCRSAT; break;
	case NV097_SET_STENCIL_OP_V_DECRSAT: op = NV_PGRAPH_CONTROL_2_STENCIL_OP_V_DECRSAT; break;
	case NV097_SET_STENCIL_OP_V_INVERT:  op = NV_PGRAPH_CONTROL_2_STENCIL_OP_V_INVERT; break;
	case NV097_SET_STENCIL_OP_V_INCR:    op = NV_PGRAPH_CONTROL_2_STENCIL_OP_V_INCR; break;
	case NV097_SET_STENCIL_OP_V_DECR:    op = NV_PGRAPH_CONTROL_2_STENCIL_OP_V_DECR; break;
	// clang-format on
	default:
		assert(false);
		break;
...
static uint32_t kelvin_map_polygon_mode(uint32_t parameter)
{
	...
	{
	// clang-format off
	case NV097_SET_FRONT_POLYGON_MODE_V_POINT: mode = NV_PGRAPH_SETUPRASTER_FRONTFACEMODE_POINT; break;
	case NV097_SET_FRONT_POLYGON_MODE_V_LINE:  mode = NV_PGRAPH_SETUPRASTER_FRONTFACEMODE_LINE; break;
	case NV097_SET_FRONT_POLYGON_MODE_V_FILL:  mode = NV_PGRAPH_SETUPRASTER_FRONTFACEMODE_FILL; break;
	// // clang-format on
	default:
		assert(false);
		break;
...
static uint32_t kelvin_map_texgen(uint32_t parameter, uint32_t channel)
{
	...
	case NV097_SET_TEXGEN_S_EYE_LINEAR:
		texgen = NV_PGRAPH_CSV1_A_T0_S_EYE_LINEAR;
		break;
	case NV097_SET_TEXGEN_S_OBJECT_LINEAR:
		texgen = NV_PGRAPH_CSV1_A_T0_S_OBJECT_LINEAR;
		break;
	case NV097_SET_TEXGEN_S_SPHERE_MAP:
		assert(channel < 2);
		texgen = NV_PGRAPH_CSV1_A_T0_S_SPHERE_MAP;
		break;
	case NV097_SET_TEXGEN_S_REFLECTION_MAP:
		assert(channel < 3);
		texgen = NV_PGRAPH_CSV1_A_T0_S_REFLECTION_MAP;
		break;
	case NV097_SET_TEXGEN_S_NORMAL_MAP:
		assert(channel < 3);
		texgen = NV_PGRAPH_CSV1_A_T0_S_NORMAL_MAP;
		break;
	default:
		assert(false);
		break;
