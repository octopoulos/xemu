...
void xbox_pci_init(
	qemu_irq* pic, MemoryRegion* address_space_mem, MemoryRegion* address_space_io, MemoryRegion* pci_memory,
	MemoryRegion* ram_memory, MemoryRegion* rom_memory, PCIBus** out_host_bus, ISABus** out_isa_bus, I2CBus** out_smbus,
	PCIBus** out_agp_bus)
{
	DeviceState* host;
	...
	host_bus = pci_root_bus_new(host, NULL, pci_memory, address_space_io, 0, TYPE_PCI_BUS);
	host_state->bus = host_bus;
	sysbus_realize_and_unref(SYS_BUS_DEVICE(host), &error_fatal);

	bridge = pci_create_simple_multifunction(host_bus, PCI_DEVFN(0, 0), true, "xbox-pci");
	...
	memory_region_init_alias(
		&bridge_state->pci_hole, OBJECT(bridge), "pci-hole", bridge_state->pci_address_space,
		memory_region_size(ram_memory), 0x100000000ULL - memory_region_size(ram_memory));
	memory_region_add_subregion(bridge_state->system_memory, memory_region_size(ram_memory), &bridge_state->pci_hole);

	/* lpc bridge */
	PCIDevice* lpc = pci_create_simple_multifunction(host_bus, PCI_DEVFN(1, 0), true, "xbox-lpc");
	XBOX_LPCState* lpc_state = XBOX_LPC_DEVICE(lpc);
	lpc_state->pic = pic;
	lpc_state->rom_memory = rom_memory;

	pci_bus_irqs(host_bus, xbox_lpc_set_irq, xbox_lpc_map_irq, lpc_state, XBOX_NUM_INT_IRQS + XBOX_NUM_PIRQS);

	qemu_irq* acpi_irq = qemu_allocate_irqs(xbox_lpc_set_acpi_irq, lpc_state, 2);
	xbox_pm_init(lpc, &lpc_state->pm, acpi_irq[0]);
	// xbox_lpc_reset(&s->dev.qdev);

	/* smbus */
	PCIDevice* smbus = pci_create_simple_multifunction(host_bus, PCI_DEVFN(1, 1), true, "xbox-smbus");
...
static void xbox_smb_ioport_writeb(void* opaque, hwaddr addr, uint64_t val, unsigned size)
{
...
static uint64_t xbox_smb_ioport_readb(void* opaque, hwaddr addr, unsigned size)
{
	XBOX_SMBState* s = opaque;
...
static void xbox_smbus_realize(PCIDevice* dev, Error** errp)
{
	...
	memory_region_init_io(&s->smb_bar, OBJECT(dev), &xbox_smbus_ops, s, "xbox-smbus-bar", 32);
	pci_register_bar(dev, XBOX_SMBUS_BASE_BAR, PCI_BASE_ADDRESS_SPACE_IO, &s->smb_bar);
}
...
static const VMStateDescription vmstate_xbox_smbus = {
	...
	.fields = (VMStateField[]) { VMSTATE_PCI_DEVICE(dev, XBOX_SMBState),
								 // FIXME
								 VMSTATE_END_OF_LIST() },
};
...
static void xbox_lpc_config_write(PCIDevice* dev, uint32_t addr, uint32_t val, int len)
{
...
static const VMStateDescription vmstate_xbox_lpc = {
	...
	.minimum_version_id = 1,
	.fields =
		(VMStateField[]) { VMSTATE_PCI_DEVICE(dev, XBOX_LPCState),
						   VMSTATE_STRUCT(pm, XBOX_LPCState, 0, vmstate_xbox_pm, XBOX_PMRegs), VMSTATE_END_OF_LIST() },
};
...
static const VMStateDescription pci_bridge_dev_vmstate = {
	.name = "xbox-pci", .fields = (VMStateField[]) { VMSTATE_PCI_DEVICE(parent_obj, PCIBridge), VMSTATE_END_OF_LIST() }
};
...
static void xbox_pcihost_realize(DeviceState* dev, Error** errp)
{
	...
	memory_region_init_io(&s->conf_mem, OBJECT(dev), &pci_host_conf_le_ops, s, "pci-conf-idx", 4);
	sysbus_add_io(sbd, CONFIG_ADDR, &s->conf_mem);
	sysbus_init_ioports(&s->busdev, CONFIG_ADDR, 4);

	memory_region_init_io(&s->data_mem, OBJECT(dev), &pci_host_data_le_ops, s, "pci-conf-data", 4);
