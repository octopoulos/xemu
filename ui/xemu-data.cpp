...
const char* xemu_get_resource_path(const char* filename)
{
	...
	if (!resource_path)
	{
		resource_path = (char*)malloc(resource_path_buffer_len);
		assert(resource_path != NULL);
	}

	// Try whichever location SDL deems appropriate
	snprintf(resource_path, resource_path_buffer_len, "%sdata/%s", sdl_base_path, filename);

	if (path_exists(resource_path))
		return resource_path;

	// Try parent directory if launched from source root
	snprintf(resource_path, resource_path_buffer_len, "%s../data/%s", sdl_base_path, filename);
	...
	snprintf(resource_path, resource_path_buffer_len, "/usr/share/xemu/data/%s", filename);
