	...
	float frame_x, float frame_y, uint32_t primary_color, uint32_t secondary_color, ControllerState* state)
{
	...
	render_decal(
		s, frame_x + 0, frame_y + 0, tex_items[obj_controller].w, tex_items[obj_controller].h,
		tex_items[obj_controller].x, tex_items[obj_controller].y, tex_items[obj_controller].w,
		tex_items[obj_controller].h, primary_color, secondary_color, 0);
	...
	for (int i = 0; i < 12; i++)
	{
		...
		if (!enabled)
			continue;
		render_decal(
			s, frame_x + buttons[i].x, frame_y + buttons[i].y, buttons[i].w, buttons[i].h, 0, 0, 1, 1, 0, 0,
			(enabled ? primary_color : secondary_color) + 0xff);
	}
	...
	render_decal(
		s, (int)(c_x - w / 2.0f + 10.0f * lstick_x), (int)(c_y - h / 2.0f + 10.0f * lstick_y), w, h,
		tex_items[obj_lstick].x, tex_items[obj_lstick].y, w, h,
		(state->buttons & CONTROLLER_BUTTON_LSTICK) ? secondary_color : primary_color,
		(state->buttons & CONTROLLER_BUTTON_LSTICK) ? primary_color : secondary_color, 0);
	...
	render_decal(
		s, (int)(c_x - w / 2.0f + 10.0f * rstick_x), (int)(c_y - h / 2.0f + 10.0f * rstick_y), w, h,
	...
	render_meter(
		s, original_frame_x + 10, original_frame_y + tex_items[obj_controller].h + 20, 150, 5, ltrig,
		primary_color + alpha, primary_color + 0xff);
	render_meter(
		s, original_frame_x + tex_items[obj_controller].w - 160, original_frame_y + tex_items[obj_controller].h + 20,
		150, 5, rtrig, primary_color + alpha, primary_color + 0xff);
...
void render_controller_port(float frame_x, float frame_y, int i, uint32_t port_color)
{
	...
	// Render port socket
	render_decal(
		s, frame_x, frame_y, tex_items[obj_port_socket].w, tex_items[obj_port_socket].h, tex_items[obj_port_socket].x,
		tex_items[obj_port_socket].y, tex_items[obj_port_socket].w, tex_items[obj_port_socket].h, port_color,
		port_color, 0);
	...
	// Render port label
	render_decal(
		s, frame_x, frame_y, tex_items[obj_port_lbl_1 + i].w, tex_items[obj_port_lbl_1 + i].h,
		tex_items[obj_port_lbl_1 + i].x, tex_items[obj_port_lbl_1 + i].y, tex_items[obj_port_lbl_1 + i].w,
...
void render_logo(uint32_t time, uint32_t primary_color, uint32_t secondary_color, uint32_t fill_color)
{
	...
	render_decal(s_logo, 0, 0, 512, 512, 0, 0, 128, 128, primary_color, secondary_color, fill_color);
	glBindVertexArray(0);
	glUseProgram(0);
}
