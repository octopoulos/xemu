...
python3 ./scripts/gen-license.py > SHURIKEN_LICENSE
git rev-parse HEAD 2>/dev/null | tr -d '\n' > SHURIKEN_COMMIT
git symbolic-ref --short HEAD > SHURIKEN_BRANCH
git describe --match 'xemu-v*' | cut -c 7- | tr -d '\n' > SHURIKEN_VERSION
tar -r --file "$tar_file" SHURIKEN_COMMIT SHURIKEN_BRANCH SHURIKEN_VERSION SHURIKEN_LICENSE
